...
HTML;
    ...
    private function renderHomepagePage(string $languageCode, string $route): string
    {
        ...
        $heroImage = $this->heroImageAsset((string) ($site['hero_image_url'] ?? ''));
    ...
    private function listingOgImage(string $entityType): string
    {
        return match ($entityType) {
            ...
            default => $this->assetUrl('/assets/images/home/hero-first-frame.webp'),
        };
    }

    private function heroImageAsset(string $value): string
    {
        $value = trim($value);
        if ($value !== '' && preg_match('/\.(webm|mp4)(\?.*)?$/i', $value) !== 1) {
            return $value;
        }

        return '/assets/images/home/hero-first-frame.webp';
    }
...
HTML;
    ...
    private function renderTemplatePage(
        ...
        $logoUrl = $this->assetUrl((string) ($site['logo_url'] ?? '/assets/images/common/logo-110.png'));
        $heroImage = $this->assetUrl($this->heroImageAsset((string) ($site['hero_image_url'] ?? '')));
        ...
        $template = strtr($template, array_merge([
            ...
            '{{hero_image_url}}' => $this->escape($this->assetUrl($this->heroImageAsset((string) ($site['hero_image_url'] ?? '')))),
    ...
    private function renderHomepageVideoSupportHtmlV2(array $site, string $languageCode): string
    {
        ...
        $poster = $this->assetUrl($this->heroImageAsset($heroImage));
