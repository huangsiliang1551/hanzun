...
final class StaticPublisher
{
    ...
    private function spawnParallelWorker(int $jobId): array
    {
        ...
        if (!is_resource($process)) {
            throw new \RuntimeException('Failed to spawn parallel site build worker');
        }
...
HTML;
    ...
    private function renderHomepagePage(string $languageCode, string $route): string
    {
        ...
        $heroImage = trim((string) ($site['hero_image_url'] ?? '')) !== '' ? (string) $site['hero_image_url'] : '/assets/images/home/hero-enterprise-showcase.webp';
        
        // Pull real backend data, filter by is_home_featured
        $products = array_values(array_filter(
            $this->cachedCollectionItems('product', $languageCode),
            fn (array $i): bool => (int) ($i['is_home_featured'] ?? 0) === 1
        ));
        $solutions = array_values(array_filter(
            $this->cachedCollectionItems('solution', $languageCode),
            fn (array $i): bool => (int) ($i['is_home_featured'] ?? 0) === 1
        ));
        $news = array_values(array_filter(
            $this->cachedCollectionItems('news', $languageCode),
            fn (array $i): bool => (int) ($i['is_home_featured'] ?? 0) === 1
        ));
        $cases = array_values(array_filter(
            $this->cachedCollectionItems('case', $languageCode),
            fn (array $i): bool => (int) ($i['is_home_featured'] ?? 0) === 1
        ));
    ...
    private function listingOgImage(string $entityType): string
    {
        return match ($entityType) {
            ...
            default => $this->assetUrl('/assets/images/home/hero-enterprise-showcase.webp'),
        };
    }
    ...
    private function cachedFooterFeaturedLinksHtml(string $entityType, string $languageCode): string
    {
        return $this->rememberFragment('shell:footer-featured:' . $entityType . ':' . $languageCode, function () use ($entityType, $languageCode): string {
            $heading = match ($entityType) {
                ...
                'solution' => $this->phrase('footer_popular_solutions', $languageCode, 'Popular Production Lines'),
                default => '',
            };
            $items = array_slice($this->cachedCollectionItems($entityType, $languageCode), 0, 4);
...
HTML;
    ...
    private function renderDropdownNavItem(string $title, string $url, array $children, string $languageCode, bool $isActive): string
    {
        ...
        $toggleLabel = $languageCode === 'zh' ? '展开' . $title : 'Toggle ' . $title;
    ...
    private function renderMegaNav(string $title, string $url, string $routeKey, string $languageCode, bool $isActive): string
    {
        ...
        $items = $this->cachedCollectionItems($entityType, $languageCode);
        // Filter: only homepage-featured items
        $items = array_values(array_filter($items, fn(array $i): bool => (int)($i['is_home_featured']??0) === 1));
        ...
        $browseLabel = $this->escape($this->phrase('product_catalog_browse', $languageCode, 'Browse by category'));
...
<div class="support-panel" hidden data-support-panel>
    <button class="support-panel-backdrop" type="button" aria-label="{$this->escape($this->phrase('close_support_panel', $languageCode, $languageCode === 'zh' ? '关闭在线客服' : 'Close support panel'))}" data-support-close></button>
    <div class="support-dialog" role="dialog" aria-modal="true" aria-label="{$title}">
        <button class="support-dialog-close" type="button" aria-label="{$this->escape($this->phrase('close_support_panel', $languageCode, $languageCode === 'zh' ? '关闭在线客服' : 'Close support panel'))}" data-support-close>
...
HTML;
    ...
    private function renderTemplatePage(
        ...
        $logoUrl = $this->assetUrl((string) ($site['logo_url'] ?? '/assets/images/common/logo-110.png'));
        $heroImage = $this->assetUrl((string) ($site['hero_image_url'] ?? '/assets/images/home/hero-enterprise-showcase.webp'));
        ...
        $template = strtr($template, array_merge([
            ...
            '{{hero_image_url}}' => $this->escape($this->assetUrl((string) ($site['hero_image_url'] ?? '/assets/images/home/hero-enterprise-showcase.webp'))),
    ...
    private function renderHomepageVideoSupportHtmlV2(array $site, string $languageCode): string
    {
        ...
        $poster = $this->assetUrl($heroImage !== '' ? $heroImage : '/assets/images/home/hero-enterprise-showcase.webp');
        $title = $this->phrase(
