...
export default function SolutionsPage() {
  ...
  async function loadSolutionDetail(id) {
    ...
    form.setFieldsValue({
      ...
      manual_asset_id:
        detail.manual_asset_id === null || detail.manual_asset_id === undefined
          ? ''
          : String(detail.manual_asset_id),
  ...
  async function handleBatchDelete() {
    ...
    try {
      ...
      message.success('已删除所选解决方案。');
      await loadSolutions(filters);
    } catch (error) {
      message.error(error.message || '删除所选解决方案失败。');
    } finally {
      setBatchSubmitting(false);
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space wrap>
          <Button size="small" onClick={() => openPublicPreview('solution', record)}>
            查看页面
          </Button>
  ...
  return (
    <>
      ...
      >
        ...
        >
          <CoverAssetField form={form} label="封面图片" helper="从资源中心选择一张图片作为解决方案封面。" />
              ...
              <div className="media-field-meta">
                <Space wrap>
                  ...
                  {manualAssetId ? (
                    ...
                    <Text type="secondary">已选资源 ID：{manualAssetId || '无'}</Text>
