...
export default function SeoDashboardPage() {
  ...
  async function loadOverview() {
    ...
    try {
      ...
      message.error(error.message || '加载 SEO 数据看板失败');
    } finally {
      setLoading(false);
    }
  }
  ...
  const statCards = useMemo(
    () => [
      ...
      { label: '可索引路由', value: routeSummary.index_count || 0 },
  ...
  const failedJobColumns = [
    { title: '对象', dataIndex: 'entity_label', width: 140, render: (value) => value || '-' },
    ...
    <PagePlaceholder hideHeader compact>
      <Space direction="vertical" size={16} style={{ width: '100%' }} className="seo-dashboard-page">
        <Alert
          type={riskLevel}
          showIcon
          message={riskLevel === 'warning' ? 'SEO 需要处理' : 'SEO 状态正常'}
          description={
            riskLevel === 'warning'
              ? '当前存在失败的 SEO 任务或未处理的 404 路径，请前往任务中心继续重试或清理。'
          ...
          <Row gutter={[16, 16]}>
            {statCards.map((item) => (
              <Col xs={24} sm={12} xl={4} key={item.label}>
                <Card className="page-card seo-dashboard-card" bordered={false}>
                  <Statistic
                    title={item.label}
                    value={item.value}
                    formatter={(value) => formatNumber(value)}
                    valueStyle={{ fontSize: 26, lineHeight: 1.2 }}
                  />
                </Card>
              </Col>
            ))}
            ...
            <Col xs={24} xl={8}>
              <Card className="page-card seo-dashboard-card" title="路由覆盖" bordered={false}>
                <Space direction="vertical" size={10} style={{ width: '100%' }}>
                  ...
                  <Statistic title="可索引" value={formatNumber(routeSummary.index_count || 0)} />
                  <Statistic
                    title="不可索引"
