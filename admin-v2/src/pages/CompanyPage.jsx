...
export default function CompanyPage() {
  ...
  async function loadAboutPageDetail(pageId) {
    ...
    try {
      ...
      message.error(error.message || '加载公司介绍页详情失败。');
    } finally {
  ...
  async function loadAboutPages(preferredId = currentPageId) {
    ...
    try {
      ...
      message.error(error.message || '加载公司介绍页列表失败。');
    } finally {
  ...
  function handleSelectBlock(blockId) {
    try {
      ...
      message.error(error.message || '请先修复配置后再切换区块。');
    }
  }
  ...
  function handleAddBlock() {
    try {
      ...
      message.error(error.message || '请先修复配置后再新增区块。');
    }
  }
  ...
  function handleAddBlockFromTemplate(templateKey) {
    ...
    try {
      ...
      message.error(error.message || '请先修复配置后再添加模板。');
    }
  }
  ...
  function moveBlock(direction) {
    ...
    try {
      ...
      message.error(error.message || '请先修复配置后再排序。');
    }
  }
  ...
  function deleteBlock() {
    ...
    try {
      ...
      message.error(error.message || '请先修复配置后再删除。');
    }
  }
  ...
  async function handleSaveBlocks() {
    ...
    try {
      ...
      message.success('公司介绍页区块已保存。');
    } catch (error) {
      message.error(error.message || '保存公司介绍页失败。');
    } finally {
      setSaving(false);
    }
  }
