...
function renderBusinessStatus(status) {
  const labelMap = {
    on_sale: '在售',
    off_sale: '停售',
    discontinued: '停产',
  ...
  return <Tag>{labelMap[status] || status || '-'}</Tag>;
}
...
export default function ProductsPage() {
  ...
  async function loadLookups() {
    try {
      const data = await getProductLookups();
      setCategories(Array.isArray(data?.categories) ? data.categories : []);
    } catch (error) {
      message.error(error.message || '加载产品分类失败。');
    }
  }
  ...
  async function loadProductDetail(id) {
    const payload = await getProductBootstrap(id);
    ...
    form.setFieldsValue({
      ...
      business_status: detail.business_status || 'on_sale',
  ...
  async function openEditDrawer(record) {
    ...
    try {
      await loadProductDetail(record.id);
    } catch (error) {
  ...
  async function handlePublishToggle(record, publishStatus) {
    ...
    try {
      await updateProductPublishStatus(record.id, publishStatus);
  ...
  async function handleRestoreLive() {
    ...
    try {
      await restoreProductLive(editingId);
      message.success('已从线上版本恢复草稿。');
      await loadProductDetail(editingId);
      await loadProducts(filters);
    } catch (error) {
  ...
  async function handleBatchPublish(publishStatus) {
    ...
    try {
      await batchUpdateProductPublishStatus(selectedRowKeys, publishStatus);
  ...
  async function handleBatchDelete() {
    ...
    try {
      ...
      message.success('已删除所选产品。');
      await loadProducts(filters);
    } catch (error) {
      message.error(error.message || '删除所选产品失败。');
    } finally {
      setBatchSubmitting(false);
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      key: 'cover',
      width: 92,
      render: (_, record) => <ContentCoverThumb record={record} label="产品" />,
    },
    {
      title: '产品名称',
      dataIndex: 'name_zh',
      ...
      render: (value, record) => (
        <Space direction="vertical" size={2} className="content-title-cell">
          <button type="button" className="content-title-link" onClick={() => openPublicPreview('product', record)}>
            <span className="content-title-primary">{value || '-'}</span>
          </button>
          <span className="content-title-secondary">{record.category_name || '未分类'}</span>
        </Space>
      ),
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space wrap>
          <Button size="small" onClick={() => openPublicPreview('product', record)}>
            查看页面
          </Button>
    ...
    <>
        ...
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
          <div className="toolbar-surface">
            <Space wrap size={12} style={{ width: '100%', justifyContent: 'space-between' }}>
              ...
              <Space wrap size={8}>
                <Button onClick={() => loadProducts(filters)}>刷新列表</Button>
      ...
      >
        ...
        >
          ...
          <Form.Item name="business_status" label="销售状态">
            <Select options={businessStatusOptions.slice(1)} />
          </Form.Item>
          ...
          <Form.Item name="content_zh" label="详细内容">
            <LazyRichTextEditor active={drawerOpen} minHeight={360} placeholder="请输入产品详细内容。" />
          </Form.Item>
