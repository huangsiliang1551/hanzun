...
export default function AccountPanel() {
  ...
  async function handleSubmit(values) {
    ...
    try {
      ...
      if (Number(result?.require_relogin || 0) === 1) {
        message.success('账户信息已更新，请重新登录后继续使用');
      } else {
      ...
      message.error(error.message || '更新账户信息失败');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <Space direction="vertical" size={16} style={{ width: '100%' }}>
      <Row gutter={[16, 16]}>
        <Col xs={24} xl={14}>
          <Card bordered={false} title="账号资料" loading={loading}>
        ...
        <Col xs={24} xl={10}>
          <Card bordered={false} title="最近登录记录" loading={loading}>
            <Alert
              type="info"
              showIcon
