...
export default function AdminUsersPanel() {
  ...
  async function loadPanel() {
    ...
    try {
      ...
      message.error(error.message || '加载管理员列表失败，请稍后重试。');
    } finally {
      setLoading(false);
    }
  }
  ...
  async function openEditDrawer(record) {
    ...
    try {
      ...
      message.error(error.message || '加载管理员详情失败，请稍后重试。');
      setDrawerOpen(false);
      setEditingId(null);
    } finally {
  ...
  async function handleSubmit(values) {
    ...
    try {
      ...
      message.error(error.message || '保存管理员失败，请稍后重试。');
    } finally {
      setSubmitting(false);
    }
  }
  ...
  async function handleDelete(record) {
    try {
      ...
      message.error(error.message || '删除管理员失败，请稍后重试。');
    }
  }
