...
export default function RichTextEditor({
  ...
  async function handleInsertAsset(asset) {
    ...
    const html = buildAssetHtml(asset);
    ...
    try {
      const inserted = isVideoAsset(asset)
        ? insertManagedVideo(editor, asset)
        : insertManagedImage(editor, asset);

      if (!inserted) {
        if (!editor.insertHTML) {
          message.error('当前编辑器不支持插入素材。');
          return;
        }
