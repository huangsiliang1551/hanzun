...
function renderEventLabel(action) {
  ...
  return labelMap[action] || action || '事件';
}

export default function ContentWorkflowPanel({ workflow, detail, loading = false, restoring = false, onRestore }) {
  if (!detail && !workflow) {
    return null;
  }
      ...
      extra={
        hasLiveSnapshot ? (
          <Popconfirm
            ...
            onConfirm={onRestore}
          >
            <Button size="small" loading={restoring}>恢复线上</Button>
          </Popconfirm>
        ) : null
      }
    >
      <Space direction="vertical" size={12} style={{ width: '100%' }}>
        <Alert
          ...
          description={workflow?.live_updated_at ? `线上更新时间：${workflow.live_updated_at}` : '当前内容还没有可恢复的线上快照。'}
        />
        ...
        <Descriptions size="small" column={2} bordered>
          <Descriptions.Item label="草稿更新时间">{workflow?.draft_updated_at || detail?.updated_at || '-'}</Descriptions.Item>
