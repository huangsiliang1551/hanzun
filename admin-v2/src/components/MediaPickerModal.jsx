...
export default function MediaPickerModal({
  ...
  function toggleSelectedAsset(item) {
    ...
    setSelectedAssets((current) => {
      ...
      const exists = current.some((a) => Number(a.id || 0) === id);
      if (exists) {
        return current.filter((a) => Number(a.id || 0) !== id);
      }
      return [...current, item];
    });
  }
  ...
  function isAssetSelected(item) {
    const id = Number(item?.id || 0);
    if (id <= 0) return false;
    if (multiple) {
      return selectedAssets.some((a) => Number(a.id || 0) === id);
    }
    return Number(selectedAsset?.id || 0) === id;
  }
        ...
        try {
          ...
          if (multiple) {
            setSelectedAssets((current) => {
              ...
              if (id > 0 && !current.some((a) => Number(a.id || 0) === id)) {
  ...
  const okTextValue = multiple
    ? `使用所选 ${selectedAssets.length} 个资源`
    : '使用当前资源';
  const okDisabled = multiple
    ? activeUploadCount > 0 || selectedAssets.length === 0
    : activeUploadCount > 0 || !selectedAsset;
  ...
  return (
    <Modal
      ...
      okButtonProps={{ loading: confirming, disabled: okDisabled }}
          ...
          <Alert
            ...
            message={
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <Text strong>已选 {selectedAssets.length} 个资源</Text>
                <Button type="link" size="small" onClick={clearSelectedAssets}>
                  清空选择
                </Button>
              </div>
            }
                ...
                return (
                  <List.Item>
                    <Card
                      ...
                      onClick={() => toggleSelectedAsset(item)}
                    >
                      {multiple && isSelected && (
                        <div className="media-picker-card-check">
            ...
            {multiple ? (
              selectedAssets.length > 0 ? (
                <Space direction="vertical" size={12} style={{ width: '100%' }}>
                  <Text strong>已选 {selectedAssets.length} 个资源</Text>
                  <div className="media-picker-multi-preview">
                    {selectedAssets.map((asset) => (
                      <div key={asset.id} className="media-picker-multi-item">
                        <div className="media-picker-multi-thumb">
                          {renderAssetPreview(asset)}
                        </div>
                        <Text
                          type="secondary"
                          ellipsis={{ tooltip: getAssetDisplayName(asset) }}
                          style={{ fontSize: 12 }}
                        >
                ...
                <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="请选择左侧资源" />
              )
            ) : selectedAsset ? (
              <Space direction="vertical" size={16} style={{ width: '100%' }}>
