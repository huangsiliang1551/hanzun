...
final class DeepSeekClient
{
    ...
    public function jsonChat(array $messages, ?string $featureFlag = null): array
    {
        ...
        try {
            ...
            if ($apiKey === '') {
                throw new BusinessException('请填写 DashScope API Key', ErrorCode::INVALID_PARAMS);
            }

            if ($featureFlag !== null && (int) ($config[$featureFlag] ?? 0) !== 1) {
                throw new BusinessException('模型名称不能为空', ErrorCode::INVALID_PARAMS);
            }
            ...
            if (!is_string($content) || trim($content) === '') {
                throw new BusinessException('DashScope 响应格式无效', ErrorCode::INTERNAL_ERROR);
            }
    ...
    private function request(string $url, string $apiKey, array $payload, int $timeoutSeconds, int $maxAttempts, int &$attempts, int &$statusCode): array
    {
        ...
        throw $lastException ?? new BusinessException('DashScope 请求失败', ErrorCode::INTERNAL_ERROR);
    }
    ...
    private function requestOnce(string $url, string $apiKey, array $payload, int $timeoutSeconds, int &$statusCode): array
    {
        ...
        if (function_exists('curl_init')) {
            ...
            if (!is_string($response) || $response === '') {
                if ($statusCode >= 400) {
                    throw new BusinessException('HTTP 请求失败: ' . $statusCode, ErrorCode::INTERNAL_ERROR);
                }

                throw new BusinessException($error !== '' ? $error : 'DashScope 请求失败', ErrorCode::INTERNAL_ERROR);
            }

            $decoded = json_decode($response, true);
            if ($statusCode >= 400) {
                $message = is_array($decoded)
                    ? (string) (($decoded['error']['message'] ?? $decoded['message'] ?? 'DashScope 请求失败'))
                    : 'DashScope 请求失败';
        ...
        if (!is_string($response) || $response === '') {
            ...
            throw new BusinessException('DashScope 请求失败', ErrorCode::INTERNAL_ERROR);
        }
    ...
    private function decodeJsonContent(string $content): array
    {
        ...
        throw new BusinessException('DashScope 响应内容解析失败', ErrorCode::INTERNAL_ERROR);
    }
