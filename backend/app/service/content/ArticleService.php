...
final class ArticleService
{
    ...
    public function create(array $input, ?array $operator): array
    {
        ...
        $this->contentWorkflowService->touchDraft('article', $record, $operator, '文章草稿已创建');
        $this->operationLogService->recordCurrentAction('article', 'article.create', 'article', $record, '文章已创建');
    ...
    public function update(int $id, array $input, ?array $operator): array
    {
        ...
        $this->contentWorkflowService->touchDraft('article', $updated, $operator, '文章草稿已更新');
        $this->operationLogService->recordCurrentAction('article', 'article.update', 'article', $updated, '文章已更新');
    ...
    public function publish(int $id, string $publishStatus, ?array $operator): array
    {
        ...
        if ($publishStatus === 'published') {
            ...
            $this->contentWorkflowService->touchDraft('article', $updated, $operator, '文章发布状态已切换为草稿或下线');
        }
        $this->operationLogService->recordCurrentAction('article', 'article.publish', 'article', $updated, '文章发布状态已更新');
    ...
    public function batchPublish(array|string $ids, string $publishStatus, ?array $operator): array
    {
        ...
        if ($normalizedIds === []) {
            throw new BusinessException('文章 ID 列表不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        foreach ($normalizedIds as $id) {
            ...
            if ($publishStatus === 'published') {
                ...
                $this->contentWorkflowService->touchDraft('article', $updated, $operator, '文章批量发布状态已切换为草稿或下线');
            }
    ...
    public function restoreLive(int $id, ?array $operator): array
    {
        ...
        $this->operationLogService->recordCurrentAction('article', 'article.restore_live', 'article', $restored, '文章已从线上版本恢复草稿');
    ...
    public function remove(int $id, ?array $operator): array
    {
        ...
        (new ContentCleanupService())->purgeEntity('article', $id, $deleted);
        $this->operationLogService->recordCurrentAction('article', 'article.delete', 'article', $deleted, '文章已删除');
    ...
    public function createCategory(array $input): array
    {
        ...
        $this->operationLogService->recordCurrentAction('article', 'article.category.create', 'article_category', $record, '文章分类已创建');
    ...
    public function updateCategory(int $id, array $input): array
    {
        ...
        if ($existing === null) {
            throw new BusinessException('文章分类不存在', ErrorCode::NOT_FOUND);
        }
        ...
        $this->operationLogService->recordCurrentAction('article', 'article.category.update', 'article_category', $updated, '文章分类已更新');
    ...
    public function deleteCategory(int $id): array
    {
        ...
        if ((int) ($node['content_total_count'] ?? 0) > 0) {
            throw new BusinessException('分类下仍有文章，不能删除', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('article', 'article.category.delete', 'article_category', $deleted, '文章分类已删除');
    ...
    private function normalizePayload(array $input, ?array $operator, bool $isCreate): array
    {
        ...
        if ($title === '' || !in_array($contentType, ['article', 'news', 'case', 'faq'], true)) {
            throw new BusinessException('文章标题不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function normalizeCategoryPayload(array $input): array
    {
        ...
        if ($name === '') {
            throw new BusinessException('文章分类名称不能为空', ErrorCode::INVALID_PARAMS);
        }
