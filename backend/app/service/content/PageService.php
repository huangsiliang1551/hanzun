...
final class PageService
{
    ...
    public function create(array $input, ?array $operator): array
    {
        ...
        $this->contentWorkflowService->touchDraft('page', $record, $operator, '单页草稿已创建');
        $this->operationLogService->recordCurrentAction('page', 'page.create', 'page', $record, '单页已创建');
    ...
    public function update(int $id, array $input, ?array $operator): array
    {
        ...
        $this->contentWorkflowService->touchDraft('page', $updated, $operator, '单页草稿已更新');
        $this->operationLogService->recordCurrentAction('page', 'page.update', 'page', $updated, '单页已更新');
    ...
    public function publish(int $id, string $publishStatus, ?array $operator): array
    {
        ...
        if ($publishStatus === 'published') {
            ...
            $this->contentWorkflowService->touchDraft('page', $updated, $operator, '单页发布状态已切换为草稿或下线');
        }
        $this->operationLogService->recordCurrentAction('page', 'page.publish', 'page', $updated, '单页发布状态已更新');
    ...
    public function batchPublish(array|string $ids, string $publishStatus, ?array $operator): array
    {
        ...
        if ($normalizedIds === []) {
            throw new BusinessException('单页 ID 列表不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        foreach ($normalizedIds as $id) {
            ...
            if ($publishStatus === 'published') {
                ...
                $this->contentWorkflowService->touchDraft('page', $updated, $operator, '单页批量发布状态已切换为草稿或下线');
            }
    ...
    public function restoreLive(int $id, ?array $operator): array
    {
        ...
        $this->operationLogService->recordCurrentAction('page', 'page.restore_live', 'page', $restored, '单页已从线上版本恢复草稿');
    ...
    public function remove(int $id, ?array $operator): array
    {
        ...
        (new ContentCleanupService())->purgeEntity('page', $id, $deleted);
        $this->operationLogService->recordCurrentAction('page', 'page.delete', 'page', $deleted, '单页已删除');
    ...
    private function normalizePayload(array $input, ?array $operator, bool $isCreate): array
    {
        ...
        if ($title === '' || !in_array($pageType, ['page', 'campaign', 'landing'], true)) {
            throw new BusinessException('单页标题不能为空', ErrorCode::INVALID_PARAMS);
        }
