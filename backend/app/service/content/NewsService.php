...
{
    private const ENTITY_TYPE = 'news';
    ...
    {
        $result = $this->newsRepository->list($query);
        $result['items'] = $this->attachCoverAssets($result['items'] ?? [], self::ENTITY_TYPE);

        return array_merge($result, [
            'filters' => ['publish_status', 'category_id', 'is_home_featured', 'keyword'],
        ]);
    }
    ...
    public function detail(int $id): array
    {
        ...
        if ($news === null) {
            throw new BusinessException('新闻不存在', ErrorCode::NOT_FOUND);
        }

        $news['media_gallery'] = $this->mediaGalleryRepository->list(self::ENTITY_TYPE, $id);
        $news = $this->attachSingleCoverAsset($news, $news['media_gallery'][0] ?? null);
    ...
    {
        $record = $this->newsRepository->create($this->normalizePayload($input, $operator, true));
        ...
        $this->contentWorkflowService->touchDraft(self::ENTITY_TYPE, $record, $operator, '新闻草稿已创建');
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.create', self::ENTITY_TYPE, $record, '新闻已创建');
    ...
    {
        $existing = $this->newsRepository->find($id);
        if ($existing === null) {
            throw new BusinessException('新闻不存在', ErrorCode::NOT_FOUND);
        }

        $updated = $this->newsRepository->update($id, $this->normalizePayload(array_merge($existing, $input), $operator, false));
        if ($updated === null) {
            throw new BusinessException('新闻不存在', ErrorCode::NOT_FOUND);
        }
        ...
        $this->contentWorkflowService->touchDraft(self::ENTITY_TYPE, $updated, $operator, '新闻草稿已更新');
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.update', self::ENTITY_TYPE, $updated, '新闻已更新');
    ...
    public function publish(int $id, string $publishStatus, ?array $operator): array
    {
        ...
        if ($publishStatus === 'published') {
            ...
            $this->contentWorkflowService->touchDraft(self::ENTITY_TYPE, $updated, $operator, '新闻发布状态已切换为草稿或下线');
        }
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.publish', self::ENTITY_TYPE, $updated, '新闻发布状态已更新');
    ...
    public function batchPublish(array|string $ids, string $publishStatus, ?array $operator): array
    {
        ...
        if ($normalizedIds === []) {
            throw new BusinessException('新闻 ID 列表不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        foreach ($normalizedIds as $id) {
            ...
            if ($publishStatus === 'published') {
                ...
                $this->contentWorkflowService->touchDraft(self::ENTITY_TYPE, $updated, $operator, '新闻批量发布状态已切换为草稿或下线');
            }
    ...
    public function workflow(int $id): array
    {
        ...
        return $this->contentWorkflowService->workflow(self::ENTITY_TYPE, $news);
    }
    ...
    public function restoreLive(int $id, ?array $operator): array
    {
        ...
        $restored = $this->contentWorkflowService->restoreLive(
            self::ENTITY_TYPE,
            $news,
            fn (array $payload): ?array => $this->newsRepository->update($id, $payload),
            $operator
        );
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.restore_live', self::ENTITY_TYPE, $restored, '新闻已从线上版本恢复草稿');
    ...
    public function remove(int $id, ?array $operator): array
    {
        ...
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.delete', self::ENTITY_TYPE, $deleted, '新闻已删除');
    ...
    {
        return $this->newsRepository->categoryTree(false, true);
    }
    ...
    public function createCategory(array $input): array
    {
        ...
        $record = $this->newsRepository->createCategory($payload);
        $this->sharedTranslationPipelineService->syncEntity('news_category', (int) ($record['id'] ?? 0));
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.category.create', 'news_category', $record, '新闻分类已创建');

        return $this->newsRepository->findCategory((int) ($record['id'] ?? 0)) ?? $record;
    }

    public function updateCategory(int $id, array $input): array
    {
        $existing = $this->newsRepository->findCategory($id);
        if ($existing === null) {
            throw new BusinessException('新闻分类不存在', ErrorCode::NOT_FOUND);
        }

        $payload = $this->normalizeCategoryPayload(array_merge($existing, $input));
        $this->assertValidCategoryParent((int) $payload['parent_id'], $id);

        $updated = $this->newsRepository->updateCategory($id, $payload);
        if ($updated === null) {
            throw new BusinessException('新闻分类不存在', ErrorCode::NOT_FOUND);
        }

        $this->sharedTranslationPipelineService->syncEntity('news_category', $id);
        $updated = $this->newsRepository->findCategory($id) ?? $updated;
        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.category.update', 'news_category', $updated, '新闻分类已更新');
    ...
    public function deleteCategory(int $id): array
    {
        ...
        $node = $this->findCategoryNode($this->newsRepository->categoryTree(false, true), $id);
        ...
        if ((int) ($node['content_total_count'] ?? 0) > 0) {
            throw new BusinessException('分类下仍有新闻，不能删除', ErrorCode::INVALID_PARAMS);
        }

        $deleted = $this->newsRepository->deleteCategory($id);
        if ($deleted === null) {
            throw new BusinessException('新闻分类不存在', ErrorCode::NOT_FOUND);
        }

        $this->operationLogService->recordCurrentAction(self::ENTITY_TYPE, 'news.category.delete', 'news_category', $deleted, '新闻分类已删除');
    ...
    private function normalizePayload(array $input, ?array $operator, bool $isCreate): array
    {
        ...
        if ($title === '') {
            throw new BusinessException('新闻标题不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function resolveCategoryName(int $categoryId): string
    {
        ...
        $category = $this->newsRepository->findCategory($categoryId);
    ...
    private function buildSlug(string $slug, string $fallback, string $prefix, int $excludeId = 0): string
    {
        ...
        while ($this->newsRepository->slugExists($candidate, $excludeId)) {
    ...
    private function normalizeCategoryPayload(array $input): array
    {
        ...
        if ($name === '') {
            throw new BusinessException('新闻分类名称不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function assertValidCategoryParent(int $parentId, int $currentId = 0): void
    {
        ...
        $parent = $this->newsRepository->findCategory($parentId);
        if ($parent === null) {
        ...
        if ($currentId > 0) {
            ...
            $subtree = $this->findCategoryNode($this->newsRepository->categoryTree(false), $currentId);
    ...
    private function categoryDepth(int $categoryId): int
    {
        ...
        while ($currentId > 0) {
            ...
            $depth++;
            $record = $this->newsRepository->findCategory($currentId);
            if ($record === null) {
                break;
            }
    ...
    {
        $node = $this->findCategoryNode($this->newsRepository->categoryTree(false), $categoryId);
        if ($node === null) {
            return 1;
        }
