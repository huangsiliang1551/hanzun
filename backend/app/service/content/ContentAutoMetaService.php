...
final class ContentAutoMetaService
{
    ...
    public function enrich(array $context): array
    {
        ...
        $publishStatus = (string) ($context['publish_status'] ?? 'draft');
        ...
        $summary = $this->generateSummary(
            (string) ($context['entity_type'] ?? 'content'),
            $title,
            $categoryName,
            $content
        );
        $generatedSeo = $this->generateSeo(
            (string) ($context['entity_type'] ?? 'content'),
            $title,
            $categoryName,
            $summary,
        ...
        $seoTitle = $generatedSeo['seo_title'];
        $seoKeywords = $generatedSeo['seo_keywords'];
        $seoDescription = $generatedSeo['seo_description'];
    ...
    {
        $plainText = $this->plainText($content);
        $systemPrompt = $this->systemSettingRepository->deepseekPrompt('seo');
        if ($systemPrompt === '') {
