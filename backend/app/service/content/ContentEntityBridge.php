...
final class ContentEntityBridge
{
    ...
    public function deleteTranslations(string $entityType, int $entityId): void
    {
        if ($entityId <= 0 || !$this->supports($entityType)) {
            return;
        }

        [$table, $entityKey] = $this->translationMeta($entityType);
        $path = $this->translationStoragePath($table);
        $pdo = DatabaseManager::instance()->connection();
        if ($pdo instanceof PDO) {
            try {
                $statement = $pdo->prepare(sprintf('DELETE FROM %s WHERE %s = :entity_id', $table, $entityKey));
        ...
        if ($path !== '' && is_file($path)) {
            ...
            if (is_array($decoded)) {
                $filtered = array_values(array_filter($decoded, static fn (mixed $row): bool => !is_array($row) || (int) ($row[$entityKey] ?? 0) !== $entityId));
                file_put_contents($path, json_encode($filtered, JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES | JSON_PRETTY_PRINT));
            }
        }
    }

    public function deleteTranslationsByLanguage(string $languageCode): void
    {
        ...
        foreach ($entityTypes as $entityType) {
            [$table] = $this->translationMeta($entityType);
            if ($pdo instanceof PDO) {
                try {
                    $statement = $pdo->prepare(sprintf('DELETE FROM %s WHERE language_code = :language_code', $table));
                    $statement->execute(['language_code' => $languageCode]);
                } catch (\Throwable) {
                }
            }
            ...
            $filtered = array_values(array_filter($decoded, static fn (mixed $row): bool => !is_array($row)
    ...
    private function translationMeta(string $entityType): array
    {
        return match ($entityType) {
            ...
            'contact_item' => ['contact_item_translations', 'contact_item_id', ['label' => 'label', 'description' => 'description', 'value' => 'value']],
            ...
            default => throw new \InvalidArgumentException('不支持的内容实体类型'),
        };
    }
