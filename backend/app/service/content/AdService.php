...
final class AdService
{
    ...
    public function saveAll(array $items): array
    {
        ...
        foreach (array_values($items) as $index => $item) {
            if (!is_array($item)) {
                throw new BusinessException('广告配置项格式无效', ErrorCode::INVALID_PARAMS);
            }
    ...
    private function normalizeItem(array $item, int $index): array
    {
        ...
        if ($linkedPage === null) {
            throw new BusinessException('关联页面不存在', ErrorCode::INVALID_PARAMS);
        }

        $linkedPageSlug = trim((string) ($linkedPage['slug'] ?? ''));
        if ($linkedPageSlug === '') {
            throw new BusinessException('关联页面 slug 不能为空', ErrorCode::INVALID_PARAMS);
        }

        $linkedPageTitle = trim((string) ($linkedPage['title_zh'] ?? ''));
        if ($linkedPageTitle === '') {
            throw new BusinessException('关联页面标题不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function validateRequiredText(string $value, int $maxLength, string $field): string
    {
        ...
        if ($value === '' || mb_strlen($value) > $maxLength) {
            throw new BusinessException($field . ' 不能为空或长度超限', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function validateAssetPath(string $value): string
    {
        $value = trim($value);
        if ($value === '') {
            throw new BusinessException('广告图片不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        if (filter_var($value, FILTER_VALIDATE_URL) === false) {
            throw new BusinessException('广告图片地址格式无效', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function normalizeFlag(mixed $value): int
    {
        ...
        throw new BusinessException('布尔值参数无效', ErrorCode::INVALID_PARAMS);
    }
}
