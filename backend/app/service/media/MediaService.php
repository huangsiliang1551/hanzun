...
final class MediaService
{
    ...
    /**
     * 强制把写入 uploads 的文件/目录权限规范化为 0666 / 0777。
     *
     * 背景: PHP mkdir($path, 0777) 与 move_uploaded_file / copy / imagejpeg 等写入
     * 实际权限 = 期望权限 & ~umask。当 PHP-FPM(www) umask=022 时，文件变 0644、
     * 目录变 0755；若被 root CLI 进程写入则属主还会变 root。下次 www 想覆盖时
     * 就会 Permission denied。此方法在每次写入后主动 chmod 兜底，确保跨用户可写。
     */
    private function ensureWritablePermissions(string $path, bool $isDir = false): void
    {
        if ($path === '' || !file_exists($path)) {
            return;
        }
        $mode = $isDir ? 0777 : 0666;
        @chmod($path, $mode);
    ...
    public function assertMimeAllowed(string $category, string $mimeType): void
    {
        ...
        if (!in_array($mimeType, $allowed, true)) {
            throw new BusinessException('不支持的文件类型', ErrorCode::UNSUPPORTED_FILE_TYPE);        }
    }
    ...
    public function detail(int $id): array
    {
        ...
        if ($record === null) {
            throw new BusinessException('媒体资源不存在', ErrorCode::NOT_FOUND);
        }
    ...
    public function storeUploadedFile(array $file, array $options = []): array
    {
        ...
        if ($originalFileName === '' || $tmpPath === '' || !is_file($tmpPath)) {
            throw new BusinessException('上传文件不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        if (!is_dir($targetDirectory) && !mkdir($targetDirectory, 0777, true) && !is_dir($targetDirectory)) {
            throw new BusinessException('上传目录创建失败', ErrorCode::UPLOAD_FAILED);
        }
        $this->ensureWritablePermissions($targetDirectory, true);

        $targetPath = $targetDirectory . DIRECTORY_SEPARATOR . $targetFileName;
        if (!move_uploaded_file($tmpPath, $targetPath)) {
            throw new BusinessException('文件保存失败', ErrorCode::UPLOAD_FAILED);
        }
        $this->ensureWritablePermissions($targetPath);
        ...
        $this->operationLogService->recordCurrentAction('media', 'media.upload', 'media_asset', $record, '媒体资源已上传');
    ...
    private function generateThumbnail(string $sourcePath, string $uuid, string $extension): ?string
    {
        ...
        $this->ensureWritablePermissions($thumbDir, true);
        ...
        if ($saved) {
            $this->ensureWritablePermissions($thumbPath);
        }
    ...
    private function optimizeImageForWeb(string $filePath, string $extension): ?array
    {
        ...
        $this->ensureWritablePermissions($filePath);
    ...
    private function persistFile(string $sourcePath, string $originalFileName, string $folderName, string $altText, string $description, int $status): array
    {
        ...
        if (!copy($sourcePath, $targetPath)) {
            throw new BusinessException('文件复制失败', ErrorCode::UPLOAD_FAILED);
        }
        $this->ensureWritablePermissions($targetPath);
        ...
        $record = $this->mediaRepository->create($payload);
        $this->operationLogService->recordCurrentAction('media', 'media.create', 'media_asset', $record, '媒体资源已创建');
    ...
    public function update(int $id, array $input): array
    {
        ...
        $this->operationLogService->recordCurrentAction('media', 'media.update', 'media_asset', $updated, '媒体资源已更新');
    ...
    public function updateStatus(int $id, int $status): array
    {
        ...
        $this->operationLogService->recordCurrentAction('media', 'media.status.update', 'media_asset', $updated, '媒体资源状态已更新');
    ...
    public function remove(int $id): array
    {
        ...
        if ($referenceCount > 0) {
            throw new BusinessException('该媒体资源仍被引用，不能删除', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('media', 'media.delete', 'media_asset', $deleted, '媒体资源已删除');
    ...
    public function rename(int $id, string $newFileName): array
    {
        ...
        if (mb_strlen($newFileName) > 255) throw new BusinessException('文件名长度不能超过 255 个字符', ErrorCode::INVALID_PARAMS);

        $asset = $this->mediaRepository->find($id);
        if (!$asset) throw new BusinessException('媒体资源不存在', ErrorCode::NOT_FOUND);

        $ext = pathinfo($newFileName, PATHINFO_EXTENSION);
        $oldExt = $asset['file_ext'] ?? '';
        if ($ext !== '' && strtolower($ext) !== strtolower($oldExt)) {
            throw new BusinessException('文件扩展名不允许修改', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function resolveAllowedSourcePath(string $sourcePath): string
    {
        ...
        if ($sourcePath === '') {
            throw new BusinessException('source_path 不能为空', ErrorCode::INVALID_PARAMS);
        }

        if (str_starts_with($sourcePath, 'http://') || str_starts_with($sourcePath, 'https://')) {
            throw new BusinessException('source_path 必须为站内 URL', ErrorCode::INVALID_PARAMS);
        }

        if (str_contains($sourcePath, '..')) {
            throw new BusinessException('source_path 不允许包含非法路径', ErrorCode::INVALID_PARAMS);
        }

        $realPath = realpath($sourcePath);
        if ($realPath === false || !is_file($realPath) || !is_readable($realPath)) {
            throw new BusinessException('source_path 对应文件不存在或不可读', ErrorCode::INVALID_PARAMS);
        }
        ...
        throw new BusinessException('source_path 不在允许的导入目录内', ErrorCode::INVALID_PARAMS);
    }
    ...
    private function normalizeUploadFileName(string $fileName): string
    {
        ...
        if ($fileName === '') {
            throw new BusinessException('file_name 不能为空', ErrorCode::INVALID_PARAMS);
        }

        $fileName = basename($fileName);
        if (preg_match('/^[A-Za-z0-9._-]{1,120}$/', $fileName) !== 1) {
            throw new BusinessException('file_name 格式无效', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function createTempSourceFileFromBase64(string $fileName, string $inlineContent): string
    {
        ...
        if (!is_string($payload) || $payload === '') {
            throw new BusinessException('file_content_base64 不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        if ($estimatedSize > 0 && $maxAllowed > 0 && $estimatedSize > $maxAllowed) {
            throw new BusinessException('Base64 文件内容超过大小限制', ErrorCode::FILE_TOO_LARGE);
        }

        $binaryContent = base64_decode($payload, true);
        if ($binaryContent === false || $binaryContent === '') {
            throw new BusinessException('file_content_base64 不是有效的 Base64 数据', ErrorCode::INVALID_PARAMS);
        }

        $extension = $this->extractExtension($fileName);
        $temporaryPath = tempnam(sys_get_temp_dir(), 'cms-media-');
        if ($temporaryPath === false) {
            throw new BusinessException('临时文件创建失败', ErrorCode::UPLOAD_FAILED);
        }

        $targetTemporaryPath = $temporaryPath . '.' . $extension;
        if (!@rename($temporaryPath, $targetTemporaryPath)) {
            @unlink($temporaryPath);
            throw new BusinessException('Base64 内容写入临时文件失败', ErrorCode::UPLOAD_FAILED);
        }

        if (file_put_contents($targetTemporaryPath, $binaryContent, LOCK_EX) === false) {
            @unlink($targetTemporaryPath);
            throw new BusinessException('媒体文件保存失败', ErrorCode::UPLOAD_FAILED);
        }
    ...
    private function normalizeFolderName(string $folderName): string
    {
        ...
        if (mb_strlen($folderName) > 50 || str_contains($folderName, '..') || preg_match('/[\r\n\t\x00-\x1F\x7F]/u', $folderName) === 1) {
            throw new BusinessException('文件夹名称格式无效', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function normalizeStatus(mixed $status): int
    {
        ...
        throw new BusinessException('状态值无效', ErrorCode::INVALID_PARAMS);
    }
    ...
    private function assertFileSizeAllowed(string $sourcePath, string $category): void
    {
        ...
        if ($limit > 0 && $size > $limit) {
            throw new BusinessException('文件大小超过限制', ErrorCode::FILE_TOO_LARGE);
        }
    }
    ...
    private function extractExtension(string $fileName): string
    {
        ...
        if ($extension === '') {
            throw new BusinessException('文件扩展名不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function assertExtensionAllowed(string $fileName, string $extension): void
    {
        ...
        foreach ($parts as $part) {
            if (in_array($part, $blocked, true)) {
                throw new BusinessException('文件包含被禁止的扩展名', ErrorCode::UNSUPPORTED_FILE_TYPE);
            }
        }
        ...
        if (!in_array($extension, $allowedExtensions, true)) {
            throw new BusinessException('不支持的文件扩展名', ErrorCode::UNSUPPORTED_FILE_TYPE);
        }
    }
    ...
    public function createFolder(array $input): array
    {
        ...
        if ($name === '') throw new BusinessException('文件夹名称不能为空', ErrorCode::INVALID_PARAMS);
        if (mb_strlen($name) > 64) throw new BusinessException('文件夹名称长度不能超过 64 个字符', ErrorCode::INVALID_PARAMS);
    ...
    public function updateFolder(int $id, array $input): array
    {
        ...
        if (!$folder) throw new BusinessException('文件夹不存在', ErrorCode::NOT_FOUND);
        $parentId = max(0, (int) ($input['parent_id'] ?? ($folder['parent_id'] ?? 0)));
        if ($parentId === $id) {
            throw new BusinessException('父级文件夹不能选择自己', ErrorCode::INVALID_PARAMS);
        }
        if ($parentId > 0 && $this->mediaRepository->findFolder($parentId) === null) {
            throw new BusinessException('目标父级文件夹不存在', ErrorCode::NOT_FOUND);
        }
    ...
    public function deleteFolder(int $id): array
    {
        ...
        if (!empty($children)) throw new BusinessException('文件夹下仍有子文件夹，不能删除', ErrorCode::INVALID_PARAMS);
        $counts = $this->mediaRepository->assetCountsPerFolder();
        if (($counts[$id] ?? 0) > 0) throw new BusinessException('文件夹下仍有媒体文件，不能删除', ErrorCode::INVALID_PARAMS);
    ...
    {
        if (empty($ids)) throw new BusinessException('ids 不能为空', ErrorCode::INVALID_PARAMS);
        $targetFolder = $targetFolderId > 0 ? $this->mediaRepository->findFolder($targetFolderId) : null;
        if ($targetFolderId > 0 && !$targetFolder) throw new BusinessException('目标文件夹不存在', ErrorCode::NOT_FOUND);
        ...
        foreach ($ids as $id) {
            ...
            if ($category === 'image' && !empty($asset['thumb_url'])) {
                ...
                if (is_file($thumbSource)) {
                    if (@copy($thumbSource, $thumbTarget)) {
                        $this->ensureWritablePermissions($thumbTarget);
                    }
