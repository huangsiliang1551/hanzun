...
final class AuthService
{
    ...
    public function login(string $username, string $password): array
    {
        ...
        if ($user === null) {
            ...
            $this->operationLogService->recordLoginAttempt($username, false, null, '用户不存在');
            throw new BusinessException('用户名或密码错误', ErrorCode::UNAUTHORIZED);
        }

        if (!$this->adminUserRepository->verifyPassword($user, $password)) {
            $this->recordLoginFailure($username);
            $this->operationLogService->recordLoginAttempt($username, false, (int) ($user['id'] ?? 0), '密码错误');
            throw new BusinessException('用户名或密码错误', ErrorCode::UNAUTHORIZED);
        }

        if ((int) ($user['status'] ?? 0) !== 1) {
            $this->recordLoginFailure($username);
            $this->operationLogService->recordLoginAttempt($username, false, (int) ($user['id'] ?? 0), '账号已禁用');
        ...
        $loginIp = $this->requestIp();
        $this->adminUserRepository->updateLoginMetadata((int) $user['id'], $loginAt, $loginIp);
