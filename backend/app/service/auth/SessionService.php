...
final class SessionService
{
    ...
    {
        $this->revokeAllForUserExcept($userId, '');
    }

    /**
     * Revoke all sessions for a user except the one matching $exceptSessionCode.
     * When $exceptSessionCode is empty, behaves identically to revokeAllForUser.
     */
    public function revokeAllForUserExcept(int $userId, string $exceptSessionCode): void
    {
        if ($userId <= 0) {
            return;
        }

        $exceptSessionCode = trim($exceptSessionCode);

        if (RuntimeStorage::enabled()) {
            $this->sessionStore()->transaction(function (array $sessions) use ($userId, $exceptSessionCode): array {
                foreach ($sessions as $sessionCode => $session) {
                    if (!is_array($session) || (int) ($session['user_id'] ?? 0) !== $userId) {
                        continue;
                    }

                    if ($exceptSessionCode !== '' && $sessionCode === $exceptSessionCode) {
                        continue;
                    }
        ...
        $pdo = DatabaseManager::instance()->connection();
        if ($pdo instanceof PDO) {
            if ($exceptSessionCode !== '') {
                $statement = $pdo->prepare(
                    'UPDATE admin_login_sessions
                     SET revoked_at = NOW()
                     WHERE user_id = :user_id
                       AND session_code <> :except
                       AND revoked_at IS NULL'
                );
                $statement->execute([
                    'user_id' => $userId,
                    'except' => $exceptSessionCode,
                ]);
            } else {
                $statement = $pdo->prepare(
                    'UPDATE admin_login_sessions
