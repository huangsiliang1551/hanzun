...
final class PublicChatService
{
    ...
    public function recordVisitorEvent(array $input): array
    {
        ...
        $languageCode = $this->resolveInputLanguage(
            (string) ($input['language'] ?? ''),
            (string) ($input['accept_language'] ?? '')
        );
        $countryCode = $this->resolveCountryCode(
            (string) ($input['country_code'] ?? ''),
            (string) ($input['header_country_code'] ?? ''),
            (string) ($input['language'] ?? ''),
            (string) ($input['accept_language'] ?? '')
        );
    ...
    public function chat(array $input): array
    {
        ...
        if ($message === '') {
            throw new BusinessException('聊天消息不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        if ($page !== '' && $this->publicChatRepository->listVisitorEvents($sessionCode) === []) {
        ...
        $conversation = $this->publicChatRepository->createOrTouchConversation([
            ...
            'country_code' => $countryCode,
            'device_type' => $this->detectDeviceType((string) ($input['user_agent'] ?? '')),
    ...
    public function session(array $input): array
    {
        ...
        if (!$this->isSessionOwnedByClient($clientId, $sessionCode)) {
            throw new BusinessException('聊天会话不存在', ErrorCode::NOT_FOUND);
        }
    ...
    private function analyzeMessage(string $message, string $languageCode, string $page, int $sessionId): array
    {
        ...
        try {
            ...
            try {
                ...
                error_log('[PublicChatService] RAG retrieve failed: ' . $e->getMessage());
                $retrievedChunks = [];
            }
            ...
            $messages = [
                [
                    'role' => 'system',
                    'content' => $this->deepseekPrompt('chat', 'system', $languageCode),
                ],
            ];

            $ragPrompt = trim($this->deepseekPrompt('chat.rag', 'system', $languageCode));
    ...
    private function deepseekPrompt(string $feature, string $template = 'system', string $languageCode = 'zh'): string
    {
        $composer = new \app\service\ai\PromptComposer();

        return match ($feature) {
            'chat' => $composer->composePublicChatSystemPrompt($languageCode),
            'chat.rag' => $composer->composePublicChatRagPrompt($languageCode),
            'translation' => $composer->composeTranslationSystemPrompt(),
            default => $this->systemSettingRepository->deepseekPrompt($feature, $template),
        };
    }
    ...
    private function resolveSessionCode(string $clientId, string $incomingSessionCode): string
    {
        ...
        // 同一用户多次访问/多页访问时，先按 client_id 找回最近的活跃会话
        $clientHash = substr(sha1($clientId), 0, 8);
        $existingCode = $this->publicChatRepository->findLatestSessionCodeByClientHash($clientHash);
    ...
    private function requireSessionCode(string $sessionCode): string
    {
        ...
        if ($sessionCode === '') {
            throw new BusinessException('session_code 不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function requireClientId(string $clientId): string
    {
        ...
        if ($clientId === '') {
            throw new BusinessException('client_id 不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function requirePage(string $page): string
    {
        $page = trim($page);
        if ($page === '') {
            throw new BusinessException('path 不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function resolveInputLanguage(string $languageCode, string $acceptLanguage = ''): string
    {
        $languageCode = strtolower(trim($languageCode));
        if ($languageCode !== '') {
            return $this->normalizeLanguage($languageCode);
        }
        ...
        $segments = preg_split('/,/', $acceptLanguage) ?: [];
        foreach ($segments as $segment) {
            $candidate = trim((string) preg_replace('/;.*$/', '', $segment));
            if ($candidate === '') {
                continue;
            }
    ...
    {
        foreach ([$inputCountryCode, $headerCountryCode] as $candidate) {
            $candidate = strtoupper(trim($candidate));
            if ($candidate === '' || $candidate === 'XX' || $candidate === 'UNKNOWN') {
                continue;
            }
        ...
        foreach ([$languageCode, ...($acceptLanguage !== '' ? (preg_split('/,/', $acceptLanguage) ?: []) : [])] as $localeCandidate) {
            $resolved = $this->extractCountryCodeFromLocale((string) $localeCandidate);
            if ($resolved !== '') {
                return $resolved;
            }
        }
    ...
    {
        $locale = trim((string) preg_replace('/;.*$/', '', $locale));
        if ($locale === '') {
            return '';
        }
        ...
        for ($index = count($segments) - 1; $index >= 1; $index--) {
            $candidate = strtoupper(trim((string) $segments[$index]));
            if (preg_match('/^[A-Z]{2}$/', $candidate) === 1) {
                return $candidate;
            }
        }
