...
final class InquiryService
{
    ...
    private function normalizeLanguageCode(string $languageCode, string $acceptLanguage = ''): string
    {
    ...
    private function normalizeCountryCode(string $countryCode): string
    {
        ...
        return preg_match('/^[A-Z]{2}$/', $countryCode) === 1 ? $countryCode : '';
    }
    ...
    public function detail(int $id): array
    {
        ...
        if ($inquiry === null) {
            throw new BusinessException('询盘不存在', ErrorCode::NOT_FOUND);
        }
        ...
        return [
            ...
            'follow_ups' => $this->normalizeRows($inquiry['follow_ups'] ?? [], ['content', 'created_by', 'created_at']),
        ];
    }
    ...
    public function workbenchDetail(string $recordType, int $id): array
    {
        ...
        throw new BusinessException('询盘状态无效', ErrorCode::INVALID_PARAMS);
    }
    ...
    public function update(int $id, array $input): array
    {
        ...
        if (!in_array($targetStatus, ['new', 'contacted', 'quoting', 'won', 'closed'], true)) {
            throw new BusinessException('当前状态不允许执行该操作', ErrorCode::INVALID_STATUS_TRANSITION);
        }
        ...
        // Auto-resolve assignee name when assigned_to changes or name is missing
        if ($changes['assigned_to'] !== ($existing['assigned_to'] ?? null) || empty($existing['assigned_to_name'] ?? null)) {
            $changes['assigned_to_name'] = $this->resolveTeamMemberName($changes['assigned_to']);
        } else {
            $changes['assigned_to_name'] = $existing['assigned_to_name'] ?? null;
        }
        ...
        $this->operationLogService->recordCurrentAction('inquiry', 'inquiry.update', 'inquiry', $updated, '询盘已更新');
    ...
    public function updateStatus(int $id, string $status): array
    {
        ...
        $this->operationLogService->recordCurrentAction('inquiry', 'inquiry.status.update', 'inquiry', $updated, '询盘状态已更新');
    ...
    public function addFollowUp(int $id, array $input): array
    {
        ...
        if ($content === '') {
            throw new BusinessException('跟进内容不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('inquiry', 'inquiry.follow_up.create', 'inquiry', $updated, '询盘跟进已新增');
    ...
    public function updateWorkbenchArchiveStatus(string $recordType, int $id, string $archiveStatus): array
    {
        ...
        if (!in_array($normalizedArchiveStatus, ['active', 'archived'], true)) {
            throw new BusinessException('询盘 ID 列表不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        throw new BusinessException('无效的批量操作类型', ErrorCode::INVALID_PARAMS);
    }
    ...
    public function convertConversationToInquiry(int $sessionId, array $input = []): array
    {
        ...
        $this->operationLogService->recordCurrentAction('inquiry', 'inquiry.conversation.convert', 'inquiry', $inquiry, 'AI 对话已转为询盘');
    ...
    private function resolveTeamMemberName(?int $memberId): ?string
    {
        ...
        try {
            ...
            if ($pdo instanceof \PDO) {
                $stmt = $pdo->prepare('SELECT name_zh FROM team_members WHERE id = :id LIMIT 1');
            ...
            // Silently fall back to null if team_members lookup fails
        }
    ...
    private function normalizeInquiry(array $inquiry): array
    {
        ...
        $inquiry['follow_ups'] = $this->normalizeRows($inquiry['follow_ups'] ?? [], ['content', 'created_by', 'created_at']);
