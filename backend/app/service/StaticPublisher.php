...
final class StaticPublisher
{
    ...
    public function executeJob(int $jobId): array
    {
        ...
        foreach ($targets as $index => $target) {
            ...
            $this->siteBuildRepository->updateJob($jobId, [
                'current_step' => $this->phaseForTarget((string) ($target['page_type'] ?? '')),
            ]);
    ...
    private function createStagingOutputDir(int $jobId): string
    {
        ...
        if (!is_dir($dir) && !@mkdir($dir, 0777, true) && !is_dir($dir)) {
            throw new \RuntimeException('Failed to create staging output directory: ' . $dir);
        }
        ...
        foreach ($this->managedRootPublicFiles() as $filename) {
            ...
            if (is_file($targetPath)) {
                if (!@unlink($targetPath)) {
                    ...
                    $failures[] = [
                        'route' => '/' . $filename,
                        'message' => 'Failed to remove root file: ' . $targetPath . ' (' . (is_array($error) && isset($error['message']) ? (string) $error['message'] : 'unlink() failed') . ')',
                    ];
                }
            }
        }
    ...
    private function syncDirectory(string $sourceDir, string $targetDir): void
    {
        ...
        if (!is_array($sourceItems)) {
            throw new \RuntimeException('Failed to read staging directory: ' . $sourceDir);
        }
    ...
    private function copyFileIntoPlace(string $sourcePath, string $targetPath): void
    {
        ...
        if (!is_dir($dir) && !@mkdir($dir, 0777, true) && !is_dir($dir)) {
            throw new \RuntimeException('Failed to create target directory: ' . $dir);
        }

        if (!@copy($sourcePath, $targetPath)) {
            $error = error_get_last();
            $detail = is_array($error) && isset($error['message']) ? (string) $error['message'] : 'copy() failed';
            throw new \RuntimeException('Failed to deploy file to ' . $targetPath . ' (' . $detail . ')');
        }
    }
    ...
    private function bootstrap(): void
    {
        ...
        putenv('DISABLE_SITE_PHRASE_USAGE_TRACKING=1');
        $_ENV['DISABLE_CLI_RUNTIME_STORAGE_FALLBACK'] = '1';
        $_ENV['DISABLE_SITE_PHRASE_USAGE_TRACKING'] = '1';
        $_SERVER['DISABLE_CLI_RUNTIME_STORAGE_FALLBACK'] = '1';
        $_SERVER['DISABLE_SITE_PHRASE_USAGE_TRACKING'] = '1';
    ...
    private function renderAndWriteTarget(array $target): array
    {
        ...
        if ($content === '') {
            throw new \RuntimeException('Unsupported target page type: ' . $pageType);
        }
...
HTML;
    ...
    private function renderHomepagePage(string $languageCode, string $route): string
    {
        ...
        $heroImage = trim((string) ($site['hero_image_url'] ?? '')) !== '' ? (string) $site['hero_image_url'] : '/assets/images/home/hero-enterprise-showcase.png';
        
        $products = $this->homepageSectionItems($homepage, ['featured_products', 'product_list']);
        $solutions = $this->homepageSectionItems($homepage, ['production_lines', 'featured_solutions', 'solution_list']);
        $news = $this->homepageSectionItems($homepage, ['company_news', 'featured_articles', 'news_list']);
        $cases = $this->homepageSectionItems($homepage, ['customer_cases', 'featured_cases', 'case_list']);
        return $this->renderShellPage($languageCode, $route, [
            ...
            'template_replacements' => [
                ...
                '{{footer_featured_products_html}}' => $this->renderFooterFeaturedLinksHtml($this->phrase('footer_popular_products', $languageCode, 'Popular Products'), array_slice($this->cachedCollectionItems('product', $languageCode), 0, 5), 'product', $languageCode),
                '{{footer_featured_solutions_html}}' => $this->renderFooterFeaturedLinksHtml($this->phrase('footer_popular_solutions', $languageCode, 'Popular Production Lines'), array_slice($this->cachedCollectionItems('solution', $languageCode), 0, 5), 'solution', $languageCode),
            ],
        ]);
    }
    ...
    private function renderAboutPage(string $languageCode, string $route): string
    {
        ...
        $main .= '<div class="metrics-dashboard-copy"><div class="metrics-capability-combo">';
            $main .= '<section class="metrics-capability-section metrics-capability-flow"><h3>' . $this->escape($this->phrase('cooperation_flow_title', $languageCode, $languageCode === 'zh' ? '鍚堜綔娴佺▼' : 'Cooperation Flow')) . '</h3><div class="metrics-flow-list">';
        ...
        $main .= '</div></section>';
        if ($certificates !== []) {
            $main .= '<section class="metrics-capability-section metrics-capability-certs"><h3>' . $this->escape($this->phrase('qualifications_title', $languageCode, $languageCode === 'zh' ? '璧勮川璇佷功' : 'Qualifications')) . '</h3><div class="metrics-cert-grid about-certificate-grid">';
        ...
        if ($team !== []) {
            ...
            foreach (array_slice($team, 0, 9) as $member) {
                ...
                foreach ($this->memberActionLinks($member, $languageCode) as $action) {
                    $classSuffix = $this->salesContactClassSuffix((string) ($action['label'] ?? ''));
                    $main .= '<a class="sales-contact-link sales-contact-' . $this->escape($classSuffix) . '" href="' . $this->escape((string) $action['href']) . '"' . (str_starts_with((string) $action['href'], 'http') ? ' target="_blank" rel="noopener"' : '') . '>' . $this->escape((string) $action['label']) . '</a>';
                }
                $main .= '</div></article>';
            }
    ...
    private function renderListingPageV2(string $entityType, string $languageCode, string $route): string
    {
        ...
        $baseRoute = $this->localizedRoute($languageCode, $this->listingRouteKeyForEntity($entityType));
        $categoryLookup = $this->categoryLookupById($categories);
        $allLabel = $this->phrase('filter_all', $languageCode, $languageCode === 'zh' ? '鍏ㄩ儴' : 'All');
    ...
    private function listingOgImage(string $entityType): string
    {
        return match ($entityType) {
            ...
            default => $this->assetUrl('/assets/images/home/hero-enterprise-showcase.png'),
        };
    }
    ...
    {
        $shellReplacements = [
            '{{site_header_html}}' => $this->renderHeader($languageCode, $route),
            '{{site_footer_html}}' => $this->renderFooter($languageCode),
            '{{floating_contact_html}}' => $this->renderFloatingContacts($languageCode),
            '{{support_panel_html}}' => $this->renderSupportPanel($languageCode),
            '{{wechat_panel_html}}' => $this->renderWechatPanel($languageCode),
    ...
    private function renderHeader(string $languageCode, string $route): string
    {
        ...
        $menus = $this->cachedNavigation('header', $languageCode);
        $fallbackItems = [];
        ...
        if ($menus === []) {
            ...
            $menus = [['items' => $fallbackItems]];
        }

        if ($menus !== []) {
            $menus[0]['items'] = $this->ensureHeaderNavItems((array) ($menus[0]['items'] ?? []), $languageCode);
        }
...
{$brandSubtitleHtml}
        ...
        <button class="menu-toggle" type="button" aria-label="{$this->escape($this->phrase('open_navigation', $languageCode, $languageCode === 'zh' ? '鎵撳紑瀵艰埅' : 'Open navigation'))}" data-menu-toggle>
            <span></span><span></span>
        </button>
...
HTML;
    ...
    private function renderDropdownNavItem(string $title, string $url, array $children, string $languageCode, bool $isActive): string
    {
        ...
        $toggleLabel = $languageCode === 'zh' ? '灞曞紑' . $title : 'Toggle ' . $title;
    ...
    private function ensureHeaderNavItems(array $items, string $languageCode): array
    {
        ...
        foreach (['home', 'about', 'products', 'solutions', 'news', 'cases', 'contact'] as $key) {
            ...
            $ordered[] = array_merge($indexed[$key], $required[$key]);
        }
    ...
    {
        $payload = $this->cachedCollectionPayload($entityType, $languageCode);
        $categories = $this->flattenNavCategories(
            $this->filterRenderableCategories(is_array($payload['categories'] ?? null) ? $payload['categories'] : [])
        );
    ...
    private function renderProductMegaNav(string $title, string $url, array $branches, string $languageCode, bool $isActive = false): string
    {
        ...
        foreach ($branches as $branch) {
            ...
            $branchHtml .= '<article class="nav-tree-branch"><a class="nav-tree-branch-title" href="' . $this->escape($branchUrl) . '">' . $this->escape($branchTitle) . '</a><div class="nav-tree-leaf-list">' . $leafHtml . '</div></article>';
        }
    ...
    private function renderSupportPromptButton(string $languageCode, string $zhLabel, string $enLabel, string $zhPrompt, string $enPrompt): string
    {
        return '<button type="button" data-support-prompt="" data-zh="' . $this->escape($zhLabel) . '" data-en="' . $this->escape($enLabel) . '" data-zh-prompt="' . $this->escape($zhPrompt) . '" data-en-prompt="' . $this->escape($enPrompt) . '">' . $this->escape($languageCode === 'zh' ? $zhLabel : $enLabel) . '</button>';
    }
    ...
    private function primaryWechatId(string $languageCode): string
    {
        foreach ($this->collectScopedContacts($languageCode, ['floating_contact', 'footer', 'contact_page']) as $contact) {
            ...
            if (str_contains($kind, 'whatsapp') || str_contains($kind, 'wechat') || str_contains((string) ($contact['kind_label'] ?? ''), '寰俊')) {
    ...
    private function renderFooter(string $languageCode): string
    {
        ...
        $featuredProducts = array_slice($this->cachedCollectionItems('product', $languageCode), 0, 5);
        $featuredSolutions = array_slice($this->cachedCollectionItems('solution', $languageCode), 0, 5);
        $footerPopularProducts = $this->renderFooterFeaturedLinksHtml(
            $this->phrase('footer_popular_products', $languageCode, 'Popular Products'),
            $featuredProducts,
            'product',
            $languageCode
        );
        $footerPopularSolutions = $this->renderFooterFeaturedLinksHtml(
            $this->phrase('footer_popular_solutions', $languageCode, 'Popular Production Lines'),
            $featuredSolutions,
            'solution',
            $languageCode
        );
        $footerContacts = $this->renderFooterPrimaryContactHtml($languageCode);
...
HTML;
    ...
    private function renderFloatingContacts(string $languageCode): string
    {
        ...
        return <<<HTML
<div class="floating-contact" aria-label="{$this->escape($this->phrase('floating_contact', $languageCode, $languageCode === 'zh' ? '鑱旂郴鏂瑰紡' : 'Contact'))}" data-contact-fab>
    <div class="floating-menu" data-contact-menu>
        <button class="float-link support-chat" type="button" aria-label="{$this->escape($this->phrase('support_title', $languageCode, $languageCode === 'zh' ? '鍦ㄧ嚎瀹㈡湇' : 'Online Support'))}" data-support-trigger>
        ...
        <div class="float-link-group support" data-contact-chooser>
            <button class="float-link float-link-toggle support" type="button" aria-label="{$this->escape($this->phrase('contact_email', $languageCode, $languageCode === 'zh' ? '閭' : 'Email'))}" data-contact-chooser-trigger aria-expanded="false">
                <span class="float-icon" aria-hidden="true">
                    <svg viewBox="0 0 24 24"><path d="M4.2 6.8h15.6a1.2 1.2 0 0 1 1.2 1.2v8a1.8 1.8 0 0 1-1.8 1.8H4.8A1.8 1.8 0 0 1 3 16V8a1.2 1.2 0 0 1 1.2-1.2Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7"/><path d="m4.3 8 7 5 8.4-5" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7"/></svg>
                </span>
        ...
        <div class="float-link-group phone" data-contact-chooser>
            <button class="float-link float-link-toggle phone" type="button" aria-label="{$this->escape($this->phrase('contact_phone', $languageCode, $languageCode === 'zh' ? '鐢佃瘽' : 'Phone'))}" data-contact-chooser-trigger aria-expanded="false">
                <span class="float-icon" aria-hidden="true">
                    <svg viewBox="0 0 24 24"><path d="M12 3.2a8.8 8.8 0 0 0-7.6 13.2L3 21l4.8-1.3A8.8 8.8 0 1 0 12 3.2Z" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.7"/><path d="M9.2 8.7c-.2-.4-.4-.4-.7-.4h-.6c-.2 0-.5.1-.7.4-.2.3-.8.9-.8 2.1 0 1.1.8 2.2.9 2.4.1.1 1.6 2.6 4 3.5 1.9.7 2.3.6 2.7.6.4-.1 1.4-.6 1.6-1.2.2-.6.2-1 .1-1.1-.1-.1-.4-.2-.8-.4s-1.1-.5-1.2-.5c-.2-.1-.4-.1-.6.2-.2.3-.7.9-.8 1-.1.2-.3.2-.6.1a6.7 6.7 0 0 1-3.3-3c-.2-.3 0-.5.1-.6.1-.1.3-.4.5-.6.1-.2.2-.3.3-.5.1-.2 0-.4 0-.5 0-.1-.4-1.1-.6-1.5Z" fill="currentColor"/></svg>
                </span>
        ...
        <a class="float-link location" href="{$this->escape($addressHref)}" aria-label="{$this->escape($this->phrase('contact_address', $languageCode, $languageCode === 'zh' ? '鍦板潃' : 'Address'))}">
            <span class="float-icon" aria-hidden="true">
                <svg viewBox="0 0 24 24"><path d="M12 4.2c-4.7 0-8.4 3-8.4 6.9 0 3.5 2.9 6.4 6.8 6.8l-.3 2.8 2.9-2.6h.2c4.1-.3 7.2-3.3 7.2-7 0-3.9-3.7-6.9-8.4-6.9Z" fill="currentColor"/><path d="M8.1 9.4v4.8h2.6M11.8 9.4v4.8M15.8 9.4h-2.4v4.8h2.4M13.4 11.8h1.9" fill="none" stroke="#ffffff" stroke-linecap="round" stroke-linejoin="round" stroke-width="1.4"/></svg>
            </span>
    ...
    <button class="floating-trigger" type="button" data-contact-trigger aria-expanded="false" aria-label="{$this->escape($this->phrase('floating_contact_hint', $languageCode, $languageCode === 'zh' ? '鐐瑰嚮鎵撳紑鑱旂郴鏂瑰紡' : 'Tap to open contact options'))}">
    ...
    <button class="back-to-top" type="button" data-back-to-top aria-label="{$this->escape($this->phrase('back_to_top', $languageCode, $languageCode === 'zh' ? '椤堕儴' : 'TOP'))}">
...
HTML;
    ...
    {
        $title = $this->escape($this->phrase('support_title', $languageCode, 'Online Support'));
        $intro = $this->escape($this->phrase('support_intro', $languageCode, 'Online support records your request and automatically organizes it into an inquiry lead.'));
        $assistant = $this->escape($this->phrase('support_assistant', $languageCode, 'Support Assistant'));
        $greeting = $this->escape($this->phrase('support_greeting', $languageCode, 'Hello. You can ask about equipment, line solutions, lead times, and quotations. You can also leave your contact details and requirements for the support assistant to organize and follow up.'));
        $placeholder = $this->escape($this->phrase('support_placeholder', $languageCode, 'Enter your question'));
        $send = $this->escape($this->phrase('button_send', $languageCode, 'Send'));

        return <<<HTML
<div class="support-panel" hidden data-support-panel>
    <button class="support-panel-backdrop" type="button" aria-label="{$this->escape($this->phrase('close_support_panel', $languageCode, $languageCode === 'zh' ? '鍏抽棴鍦ㄧ嚎瀹㈡湇' : 'Close support panel'))}" data-support-close></button>
    <div class="support-dialog" role="dialog" aria-modal="true" aria-label="{$title}">
        <button class="support-dialog-close" type="button" aria-label="{$this->escape($this->phrase('close_support_panel', $languageCode, $languageCode === 'zh' ? '鍏抽棴鍦ㄧ嚎瀹㈡湇' : 'Close support panel'))}" data-support-close>
        ...
        <div class="support-suggestions">
            {$this->renderSupportPromptButton($languageCode, $this->unicodeText('\u86cb\u7cd5\u751f\u4ea7\u7ebf'), 'Cake Lines', $this->unicodeText('\u6211\u60f3\u54a8\u8be2\u86cb\u7cd5\u751f\u4ea7\u7ebf'), 'I want to ask about cake production lines.')}
            {$this->renderSupportPromptButton($languageCode, $this->unicodeText('\u8bbe\u5907\u4ea4\u671f'), 'Lead Time', $this->unicodeText('\u6211\u60f3\u4e86\u89e3\u8bbe\u5907\u4ea4\u671f'), 'I want to know the equipment lead time.')}
            {$this->renderSupportPromptButton($languageCode, $this->unicodeText('\u6574\u7ebf\u62a5\u4ef7'), 'Line Quotation', $this->unicodeText('\u6211\u9700\u8981\u6574\u7ebf\u62a5\u4ef7'), 'I need a quotation for a complete line.')}
        </div>
        <form class="support-composer" data-support-form>
            <textarea rows="3" placeholder="{$placeholder}" data-support-input data-zh-placeholder="{$this->escape($this->unicodeText('\u8bf7\u8f93\u5165\u60a8\u7684\u95ee\u9898'))}" data-en-placeholder="Enter your question"></textarea>
...
HTML;
    ...
    private function loadPublicTemplate(): string
    {
        ...
        throw new \RuntimeException('Public template not found: ' . implode(' | ', $candidates));
    }
    ...
    private function prepareTemplateShell(string $template): string
    {
        ...
        $template = preg_replace('/<footer class="site-footer-redesign".*?<\/footer>\s*<footer class="site-footer">.*?<\/footer>/su', '{{site_footer_html}}', $template, 1) ?? $template;
    ...
    private function renderTemplatePage(
        ...
        $site = $this->cachedSite();
        $template = $this->prepareTemplateShell($template);
        ...
        $logoUrl = $this->assetUrl((string) ($site['logo_url'] ?? '/assets/images/common/logo-110.png'));
        $heroImage = $this->assetUrl((string) ($site['hero_image_url'] ?? '/assets/images/home/hero-enterprise-showcase.png'));
        $ogImage = $this->assetUrl((string) ($payload['og_image'] ?? $heroImage));
        ...
        $template = strtr($template, array_merge([
            ...
            '{{hero_image_url}}' => $this->escape($this->assetUrl((string) ($site['hero_image_url'] ?? '/assets/images/home/hero-enterprise-showcase.png'))),
        ...
        $template = $this->localizeTemplateMarkup($template, $languageCode);
    ...
    private function publicRuntimeJson(): string
    {
        $languages = array_map(function (array $language): array {
            ...
            $normalized = str_starts_with($code, 'en') ? 'en' : (str_starts_with($code, 'zh') ? 'zh' : substr($code, 0, 2));
            $native = trim((string) ($language['name'] ?? ''));

            if ($normalized === 'zh') {
                $native = '涓枃';
            } elseif ($normalized === 'en') {
                $native = 'English';
            } elseif ($native === '') {
                $native = strtoupper($normalized);
            }

            return [
                'code' => $normalized,
                'country' => $normalized === 'zh' ? 'China' : 'United Kingdom',
                'native' => $native,
                'content' => $normalized === 'zh' ? 'zh' : 'en',
                'htmlLang' => $normalized === 'zh' ? 'zh-CN' : 'en-GB',
                'continent' => $normalized === 'zh' ? 'asia' : 'europe',
            ];
        }, $this->enabledLanguages());

        if ($languages === []) {
            $languages = [
                ['code' => 'zh', 'country' => 'China', 'native' => '涓枃', 'content' => 'zh', 'htmlLang' => 'zh-CN', 'continent' => 'asia'],
                ['code' => 'en', 'country' => 'United Kingdom', 'native' => 'English', 'content' => 'en', 'htmlLang' => 'en-GB', 'continent' => 'europe'],
            ];
        }
        ...
        return json_encode([
            ...
            'defaultLanguage' => $this->defaultLanguage(),
        ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) ?: '{"languages":[{"code":"zh","country":"China","native":"涓枃","content":"zh","htmlLang":"zh-CN","continent":"asia"}],"defaultLanguage":"zh"}';
    }
    ...
    {
        $replacements = [
            '灞曞紑产品' => $this->unicodeText('\u5c55\u5f00\u4ea7\u54c1'),
            '灞曞紑方案' => $this->unicodeText('\u5c55\u5f00\u65b9\u6848'),
            '灞曞紑新闻' => $this->unicodeText('\u5c55\u5f00\u65b0\u95fb'),
            '灞曞紑案例' => $this->unicodeText('\u5c55\u5f00\u6848\u4f8b'),
            '点击打开联系方式' => $languageCode === 'zh'
                ? $this->unicodeText('\u70b9\u51fb\u6253\u5f00\u8054\u7cfb\u65b9\u5f0f')
                : 'Tap to open contact options',
        ];
    ...
    private function renderHomepageSolutionsHtml(array $items): string
    {
        ...
        foreach (array_slice($items, 0, 6) as $item) {
            ...
            $html .= '<article class="delivery-card reveal">';
            $html .= '<figure class="delivery-media"><img src="' . $this->assetUrl((string) ($item['cover_image_url'] ?? '')) . '" alt="' . $this->escape($title) . '" loading="lazy" decoding="async" data-progressive-media></figure>';
            $html .= '<h3>' . $this->escape($title) . '</h3>';
            if ($summary !== '') {
                $html .= '<p>' . $this->escape($summary) . '</p>';
            }
            $html .= '</article>';
        }
    ...
    private function renderHomepageCasesHtml(array $items, string $languageCode): string
    {
        ...
        $html .= '</article><div class="case-list">';
    ...
    private function renderHomepageNewsHtml(array $items, string $languageCode): string
    {
        ...
        foreach (array_slice($items, 0, 5) as $item) {
            ...
            $html .= '<figure class="news-media"><img src="' . $this->assetUrl((string) ($item['cover_image_url'] ?? '')) . '" alt="' . $this->escape($title) . '" loading="lazy" decoding="async" data-progressive-media></figure>';
            $html .= '<div class="news-card-copy">';
            if ($tag !== '') {
                $html .= '<span class="news-card-tag">' . $this->escape($this->excerpt($tag, 32)) . '</span>';
            }
            $html .= '<h3>' . $this->escape($title) . '</h3></div>';
            $html .= '<a class="news-card-link" href="' . $this->localizedRoute($languageCode, 'news/' . (string) ($item['slug'] ?? '')) . '" aria-label="' . $this->escape($title) . '"></a>';
            $html .= '</article>';
        }
    ...
    private function renderHomepageVideoSupportHtml(array $site, string $languageCode): string
    {
        $videoUrl = trim((string) ($site['enterprise_video_url'] ?? ''));
        $poster = $this->assetUrl((string) ($site['hero_image_url'] ?? '/assets/images/home/hero-enterprise-showcase.png'));
        $title = $this->phrase(
        ...
        $html .= '<video' . ($videoUrl !== '' ? ' data-src="' . $this->escape($this->assetUrl($videoUrl)) . '"' : '') . ' poster="' . $this->escape($poster) . '" controls playsinline webkit-playsinline x5-playsinline preload="none" data-progressive-video></video>';
        ...
        foreach ($this->serviceCapabilityLines($languageCode) as $line) {
            $html .= '<article class="service-support-item"><span class="service-support-icon" aria-hidden="true">鉁?/span><strong>' . $this->escape($line) . '</strong></article>';
        }
        $html .= '</div><a class="button button-primary service-support-button" href="#contact-form">' . $this->escape($this->phrase('button_get_solution_detail', $languageCode, $languageCode === 'zh' ? '鐐瑰嚮鑾峰彇璇︾粏鏂规' : 'Click for Detailed Solution')) . '</a>';
    ...
    private function renderHomepageNoticeHtml(array $site, string $languageCode): string
    {
        ...
        return '<section class="section section-notice-strip" id="notice-banner"><div class="container"><article class="notice-banner reveal"><figure class="notice-banner-media"><img src="' . $this->assetUrl($image) . '" alt="' . $this->escape($title) . '" loading="lazy" decoding="async" data-progressive-media></figure><div class="notice-banner-copy"><span>' . $this->escape($title) . '</span><strong>' . $this->escape($content) . '</strong></div></article></div></section>';
    }
    ...
    private function renderHomepageMetricsHtml(array $about, string $languageCode): string
    {
        ...
        $html .= '<div class="section-heading reveal"><span class="eyebrow">' . $this->escape($this->phrase('service_capability_heading', $languageCode, $languageCode === 'zh' ? '鏈嶅姟鑳藉姏' : 'Service Capability')) . '</span></div>';
        ...
        if ($intro !== '') {
            $html .= '<figcaption class="metrics-dashboard-intro"><p>' . $this->escape($intro) . '</p></figcaption>';
        }
        $html .= '</figure><div class="metrics-dashboard-copy"><div class="metrics-capability-combo">';
        $html .= '<section class="metrics-capability-section metrics-capability-flow"><h3>' . $this->escape($this->phrase('cooperation_flow_title', $languageCode, $languageCode === 'zh' ? '鍚堜綔娴佺▼' : 'Cooperation Flow')) . '</h3><div class="metrics-flow-list">';
        ...
        $html .= '</div></section>';
        $html .= '<section class="metrics-capability-section metrics-capability-certs"><h3>' . $this->escape($this->phrase('qualifications_title', $languageCode, $languageCode === 'zh' ? '璧勮川璇佷功' : 'Qualifications')) . '</h3><div class="metrics-cert-grid">';
        foreach (array_slice($certificates, 0, 5) as $item) {
            $name = trim((string) ($item['name'] ?? 'Certificate'));
    ...
    private function renderHomepageSalesHtml(array $about, string $languageCode): string
    {
        ...
        foreach ($team as $member) {
            ...
            foreach ($this->memberActionLinks($member, $languageCode) as $action) {
                $classSuffix = $this->salesContactClassSuffix((string) ($action['label'] ?? ''));
                $html .= '<a class="sales-contact-link sales-contact-' . $this->escape($classSuffix) . '" href="' . $this->escape((string) $action['href']) . '"' . (str_starts_with((string) $action['href'], 'http') ? ' target="_blank" rel="noopener"' : '') . '>' . $this->escape((string) $action['label']) . '</a>';
            }
            $html .= '</div></article>';
        }
    ...
    private function renderHomepageContactHtml(string $languageCode): string
    {
        ...
        $html = '<section class="section section-contact" id="contact-form"><div class="container contact-panel reveal"><div class="contact-form-panel"><div class="contact-panel-head"><span class="eyebrow">' . $this->escape($this->phrase('nav_contact', $languageCode, $languageCode === 'zh' ? '鑱旂郴宸ュ巶' : 'Contact Factory')) . '</span></div>';
        $html .= '<form class="lead-form" action="javascript:void(0)"><div class="lead-form-grid">';
        $html .= '<label><span>' . $this->escape($this->phrase('form_contact_name', $languageCode, '')) . '</span><input type="text" name="name" placeholder="' . $this->escape($this->phrase('form_contact_name_placeholder', $languageCode, $languageCode === 'zh' ? '璇疯緭鍏ヨ仈绯讳汉' : 'Name')) . '"></label>';
        $html .= '<label><span>' . $this->escape($this->phrase('form_phone_with_code', $languageCode, $languageCode === 'zh' ? '鍥藉鍖哄彿 + 鐢佃瘽' : 'Country Code + Phone')) . '</span><input type="tel" name="phone" placeholder=""></label>';
        ...
        $html .= '<div class="lead-form-actions"><button class="button button-primary" type="submit">' . $this->escape($this->phrase('button_submit_inquiry', $languageCode, $languageCode === 'zh' ? '鎻愪氦鑱旂郴淇℃伅' : 'Submit Inquiry')) . '</button></div></form></div>';
    ...
    private function renderHomepageVideoSupportHtmlV2(array $site, string $languageCode): string
    {
        ...
        $poster = $this->assetUrl($heroImage !== '' ? $heroImage : '/assets/images/home/hero-enterprise-showcase.png');
        $title = $this->phrase(
            'service_support_title',
            $languageCode,
            ''
    ...
    private function serviceCapabilityLines(string $languageCode): array
    {
        if ($languageCode === 'zh') {
            return [
                $this->unicodeText('OEM / ODM \u652f\u6301'),
                $this->unicodeText('\u8bd5\u673a\u9a8c\u8bc1\u652f\u6301'),
                $this->unicodeText('\u6574\u7ebf\u96c6\u6210\u652f\u6301'),
                $this->unicodeText('\u51fa\u53e3\u9879\u76ee\u652f\u6301'),
            ];
        }
        ...
        return [
            'OEM / ODM Support',
            'Trial Verification Support',
            'Integrated Line Support',
    ...
    {
        if (!env_flag('DISABLE_SITE_PHRASE_USAGE_TRACKING')) {
        ...
        return $this->sitePhraseRepository->getText($key, $languageCode, $fallback);
    }
    ...
    private function publicPhraseOverride(string $key, string $languageCode): ?string
    {
        ...
        $overrides = [
            ...
            'en' => [
                ...
                'footer_popular_solutions' => 'Popular Production Lines',
    ...
    {
        $labels = $languageCode === 'zh'
            ? [
                $this->unicodeText('\u9700\u6c42\u6c9f\u901a'),
                $this->unicodeText('\u751f\u4ea7\u5236\u9020'),
                $this->unicodeText('\u8c03\u8bd5\u4ea4\u4ed8'),
                $this->unicodeText('\u552e\u540e\u652f\u6301'),
            ]
            : ['Requirement Review', 'Production', 'Commissioning & Delivery', 'After-Sales Support'];
    ...
    private function collectPrimaryContacts(string $languageCode): array
    {
        ...
        foreach ($items as $item) {
            ...
            if ($field === 'email' || str_contains($field, 'mail') || str_contains($label, '閭') || str_contains(strtolower($label), 'email')) {
                $contacts[] = ['kind_label' => $label ?: $this->phrase('contact_email', $languageCode, 'Email'), 'label' => $value, 'href' => 'mailto:' . $value];
            } elseif ($field === 'phone' || str_contains($field, 'phone') || str_contains($label, '鐢佃瘽') || str_contains($label, '鎵嬫満') || str_contains(strtolower($label), 'phone')) {
            ...
            } elseif ($field === 'address' || str_contains($field, 'address') || str_contains($label, '鍦板潃') || str_contains(strtolower($label), 'address')) {
    ...
    private function memberActionLinks(array $member, string $languageCode): array
    {
        ...
        if ($email !== '') {
            $actions[] = ['label' => $this->phrase('contact_email', $languageCode, 'Email'), 'href' => 'mailto:' . $email];
        }
        if ($phone !== '') {
            $actions[] = ['label' => $this->phrase('contact_phone', $languageCode, 'Phone'), 'href' => 'tel:' . preg_replace('/\s+/', '', $phone)];
        }
        if ($whatsapp !== '') {
            $actions[] = ['label' => 'WhatsApp', 'href' => 'https://wa.me/' . preg_replace('/[^0-9]/', '', $whatsapp)];
        }
    ...
    private function salesContactClassSuffix(string $label): string
    {
        ...
        return match (true) {
            str_contains($normalized, 'mail'), str_contains($label, '閭') => 'email',
            str_contains($normalized, 'phone'), str_contains($label, '鐢佃瘽') => 'phone',
    ...
    private function resolveNewsDetail(string $slug, string $languageCode): array
    {
        ...
        throw new \RuntimeException('News detail not found: ' . $slug);
    }
    ...
    private function resolveCaseDetail(string $slug, string $languageCode): array
    {
        ...
        throw new \RuntimeException('Case detail not found: ' . $slug);
    }
    ...
    private function cachedSite(): array
    {
        return $this->remember('site', fn (): array => $this->siteService->site());
    }
