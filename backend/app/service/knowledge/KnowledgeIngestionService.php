...
final class KnowledgeIngestionService
{
    ...
    public function ingestDocument(int $documentId, string $rawText): array
    {
        ...
        if ($text === '') {
            ...
            throw new BusinessException('knowledge content empty', ErrorCode::INVALID_PARAMS);
        }
    ...
    public function readTextFromFile(string $filePath): string
    {
        ...
        if (!is_file($absolute)) {
            throw new BusinessException('knowledge file not found', ErrorCode::NOT_FOUND);
        }

        $extension = strtolower(pathinfo($absolute, PATHINFO_EXTENSION));
        if (!in_array($extension, ['txt', 'md', 'markdown', 'json'], true)) {
            throw new BusinessException('unsupported knowledge file type', ErrorCode::INVALID_PARAMS);
        }

        $content = file_get_contents($absolute);
        if ($content === false) {
            throw new BusinessException('cannot read knowledge file', ErrorCode::INTERNAL_ERROR);
        }
