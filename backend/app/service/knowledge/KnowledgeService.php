...
final class KnowledgeService
{
    ...
    public function documentDetail(int $id, int $chunkPreviewLimit = 20): array
    {
        ...
        if ($document === null) {
            throw new BusinessException('knowledge document not found', ErrorCode::NOT_FOUND);
        }
    ...
    public function createManual(array $input): array
    {
        ...
        if ($title === '' || $content === '') {
            throw new BusinessException('title and content required', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.knowledge.create', 'knowledge_document', (string) ($document['id'] ?? ''), 'knowledge document created');
    ...
    public function createFromUpload(array $input): array
    {
        ...
        if ($title === '' || $filePath === '') {
            throw new BusinessException('title and file_path required', ErrorCode::INVALID_PARAMS);
        }
        ...
        $indexed = $this->ingestionService->ingestDocument((int) ($document['id'] ?? 0), $content);
        $this->operationLogService->recordCurrentAction('system', 'system.knowledge.upload', 'knowledge_document', (string) ($document['id'] ?? ''), 'knowledge document uploaded');
    ...
    public function updateDocument(int $id, array $input): array
    {
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.knowledge.update', 'knowledge_document', (string) $id, 'knowledge document updated');
    ...
    public function deleteDocument(int $id): array
    {
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.knowledge.delete', 'knowledge_document', (string) $id, 'knowledge document deleted');
    ...
    public function reindexDocument(int $id, ?string $content = null): array
    {
        ...
        if ($content === null || trim($content) === '') {
            ...
            if ($sourceType === 'upload' && trim((string) ($document['file_path'] ?? '')) !== '') {
                ...
                throw new BusinessException('content required for reindex', ErrorCode::INVALID_PARAMS);
            }
        }

        $indexed = $this->ingestionService->ingestDocument($id, $content);
        $this->operationLogService->recordCurrentAction('system', 'system.knowledge.reindex', 'knowledge_document', (string) $id, 'knowledge document reindexed');
    ...
    public function syncCms(array $options = []): array
    {
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.knowledge.sync_cms', 'knowledge_document', 'cms', 'knowledge cms sync completed');
        ...
        foreach ($items as $item) {
            ...
            try {
                ...
                if (($item['source_type'] ?? '') === 'upload') {
                    ...
                    $this->syncService->syncCmsContent(['types' => [(string) ($item['source_type'] ?? '')]]);
                }
                $summary['success']++;
            } catch (\Throwable) {
                $summary['failed']++;
