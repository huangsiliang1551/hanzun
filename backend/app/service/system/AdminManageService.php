...
final class AdminManageService
{
    ...
    public function userDetail(int $id): array
    {
        ...
        if ($user === null) {
            throw new BusinessException('管理员不存在', ErrorCode::NOT_FOUND);
        }
    ...
    public function createUser(array $input): array
    {
        ...
        if ($this->adminUserRepository->findByUsername($username) !== null) {
            throw new BusinessException('管理员账号已存在', ErrorCode::ALREADY_EXISTS);
        }
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.admin_user.create', 'admin_user', $record, '管理员已创建');
    ...
    public function updateUser(int $id, array $input): array
    {
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.admin_user.update', 'admin_user', $record, '管理员已更新');
    ...
    public function deleteUser(int $id, ?array $operator): array
    {
        ...
        if ($operator !== null && (int) ($operator['id'] ?? 0) === $id) {
            throw new BusinessException('当前管理员不能删除自己', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.admin_user.delete', 'admin_user', $deleted, '管理员已删除');
    ...
    public function createRole(array $input): array
    {
        ...
        if ($this->roleRepository->findRoleByCode((string) ($payload['code'] ?? '')) !== null) {
            throw new BusinessException('角色编码已存在', ErrorCode::ALREADY_EXISTS);
        }

        $record = $this->roleRepository->createRole($payload);
        $this->operationLogService->recordCurrentAction('system', 'system.role.create', 'admin_role', $record, '角色已创建');
    ...
    public function updateRole(int $id, array $input): array
    {
        ...
        if ($existing === null) {
            throw new BusinessException('角色不存在', ErrorCode::NOT_FOUND);
        }
        ...
        if (in_array((string) ($existing['code'] ?? ''), ['super-admin', 'operator'], true)
            ...
            throw new BusinessException('当前角色不能修改为超级管理员', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.role.update', 'admin_role', $updated, '角色已更新');
    ...
    public function deleteRole(int $id): array
    {
        ...
        if (in_array((string) ($existing['code'] ?? ''), ['super-admin', 'operator'], true)) {
            throw new BusinessException('系统内置角色不能删除', ErrorCode::INVALID_PARAMS);
        }

        if ($this->roleRepository->countUsersForRole($id) > 0) {
            throw new BusinessException('角色下仍有关联管理员，不能删除', ErrorCode::INVALID_PARAMS);
        }
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.role.delete', 'admin_role', $deleted, '角色已删除');
    ...
    public function updateRolePermissions(int $id, array $input): array
    {
        ...
        $this->operationLogService->recordCurrentAction('system', 'system.role.permissions.update', 'admin_role', $role, '角色权限已更新');
    ...
    private function validatePassword(string $password, bool $required): string
    {
        ...
        if ($password === '') {
            if ($required) {
                throw new BusinessException('用户名格式不正确', ErrorCode::INVALID_PARAMS);
            }
        ...
        if (strlen($password) < 8 || strlen($password) > 64) {
            throw new BusinessException('密码长度需为 8-64 位', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function validateNickname(string $nickname, string $fallback): string
    {
        ...
        if (mb_strlen($nickname) > 50) {
            throw new BusinessException('昵称长度不能超过 50 个字符', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function validateEmail(string $email): string
    {
        ...
        if (strlen($email) > 120 || filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
            throw new BusinessException('角色列表不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function validateMobile(string $mobile): string
    {
        ...
        if (preg_match('/^[0-9+\-\s()]{6,30}$/', $mobile) !== 1) {
            throw new BusinessException('角色名称不能为空', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function normalizeRolePayload(array $input): array
    {
        ...
        if ($name === '' || mb_strlen($name) > 64) {
            throw new BusinessException('角色名称长度不能超过 64 个字符', ErrorCode::INVALID_PARAMS);
        }

        if ($code === '' || preg_match('/^[A-Za-z0-9][A-Za-z0-9._-]{1,63}$/', $code) !== 1) {
            throw new BusinessException('角色编码不能为空', ErrorCode::INVALID_PARAMS);
        }

        if (mb_strlen($description) > 255) {
            throw new BusinessException('角色描述长度不能超过 255 个字符', ErrorCode::INVALID_PARAMS);
        }
    ...
    private function normalizeRoleIds(mixed $roleIds, bool $required): array
    {
        if (!is_array($roleIds)) {
            if ($required) {
                throw new BusinessException('菜单 ID 无效', ErrorCode::INVALID_PARAMS);
            }
        ...
        if ($required && $roleIds === []) {
            throw new BusinessException('角色 ID 列表不能为空', ErrorCode::INVALID_PARAMS);
        }
        ...
        foreach ($roleIds as $roleId) {
            if (!in_array($roleId, $validRoleIds, true)) {
                throw new BusinessException('?? ID ??', ErrorCode::INVALID_PARAMS);
            }
        }
    ...
    {
        if (!is_array($ids)) {
            throw new BusinessException('权限点 ID 无效', ErrorCode::INVALID_PARAMS);
        }
