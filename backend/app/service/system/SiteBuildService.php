...
final class SiteBuildService
{
    ...
    public function createJob(array $input, ?array $operator = null): array
    {
        ...
        $job = $this->siteBuildRepository->createJob([
            ...
            'created_by' => is_array($operator) ? (string) ($operator['nickname'] ?? $operator['username'] ?? 'system') : 'system',
        ], is_array($plan['items'] ?? null) ? $plan['items'] : []);
        $job = $this->siteBuildRepository->updateJob((int) ($job['id'] ?? 0), [
            'status' => 'queued',
        ]) ?? $job;
    ...
    public function processOneJob(): array
    {
        $this->recoverStaleJobs();
        $job = $this->siteBuildRepository->currentJob();
        if ($job === null) {
        ...
        return $this->runJob((int) ($job['id'] ?? 0));
    }
    ...
    private function dispatchAsync(int $jobId): void
    {
        ...
        if ($phpBinary === '' || !\is_file($phpBinary) || !\is_executable($phpBinary) || \str_contains($phpBinaryBase, 'php-cgi') || \str_starts_with($phpBinaryBase, 'php-fpm')) {
            $phpBinary = 'php';
        }

        $logDirectory = dirname(__DIR__, 3) . '/runtime/logs';
        if (!\is_dir($logDirectory) && !@mkdir($logDirectory, 0777, true) && !\is_dir($logDirectory)) {
            $this->siteBuildRepository->updateJob($jobId, [
                'status' => 'failed',
                'current_step' => 'failed',
                'error_message' => 'failed to prepare site build worker log directory: ' . $logDirectory,
                'finished_at' => date('Y-m-d H:i:s'),
            ]);
            return;
        }

        $logFile = $logDirectory . '/site-build-worker.log';
        if ((\is_file($logFile) && !@\is_writable($logFile))
            || (!\is_file($logFile) && @\file_put_contents($logFile, '', FILE_APPEND) === false)
            || (\is_file($logFile) && @\file_put_contents($logFile, '', FILE_APPEND) === false)) {
            $this->siteBuildRepository->updateJob($jobId, [
                'status' => 'failed',
                'current_step' => 'failed',
                'error_message' => 'site build worker log is not writable: ' . $logFile,
                'finished_at' => date('Y-m-d H:i:s'),
            ]);
            return;
        }
        ...
        if (DIRECTORY_SEPARATOR === '\\') {
            ...
            $this->siteBuildRepository->updateJob($jobId, [
                'status' => 'failed',
                'current_step' => 'failed',
                'error_message' => 'failed to start site build worker via popen',
                'finished_at' => date('Y-m-d H:i:s'),
            ]);
            return;
        }
        ...
        if (!\function_exists('exec')) {
            $this->siteBuildRepository->updateJob($jobId, [
                'status' => 'failed',
                'current_step' => 'failed',
                'error_message' => 'site build worker dispatch unavailable: PHP exec() is disabled',
                'finished_at' => date('Y-m-d H:i:s'),
            ]);
            return;
        }
        ...
        $launcher = 'nohup';
        if (!\is_file('/usr/bin/nohup') || !\is_executable('/usr/bin/nohup')) {
            $launcher = '';
        }

        $dispatchLockDirectory = dirname(__DIR__, 3) . '/runtime/tmp';
        if (!\is_dir($dispatchLockDirectory) && !@mkdir($dispatchLockDirectory, 0777, true) && !\is_dir($dispatchLockDirectory)) {
            $this->siteBuildRepository->updateJob($jobId, [
                'status' => 'failed',
                'current_step' => 'failed',
                'error_message' => 'failed to prepare site build dispatch lock directory: ' . $dispatchLockDirectory,
                'finished_at' => date('Y-m-d H:i:s'),
            ]);
            return;
        }

        $dispatchLockFile = $dispatchLockDirectory . '/site-build-dispatch-' . $jobId . '.lock';
        $dispatchLockHandle = @fopen($dispatchLockFile, 'c+');
        if ($dispatchLockHandle === false) {
            $this->siteBuildRepository->updateJob($jobId, [
                'status' => 'failed',
                'current_step' => 'failed',
                'error_message' => 'failed to open site build dispatch lock file: ' . $dispatchLockFile,
                'finished_at' => date('Y-m-d H:i:s'),
            ]);
            return;
        }

        if (!@flock($dispatchLockHandle, LOCK_EX | LOCK_NB)) {
            @fclose($dispatchLockHandle);
            return;
        }

        $backgroundCommand = ($launcher !== '' ? $launcher . ' ' : '')
            . $command
            . ' >> ' . escapeshellarg($logFile) . ' 2>&1 < /dev/null & echo $!';
        $dispatchOutput = [];
        $dispatchCode = 0;
        @\exec($backgroundCommand, $dispatchOutput, $dispatchCode);

        $workerPid = isset($dispatchOutput[0]) ? (int) $dispatchOutput[0] : 0;
        usleep(500000);

        $job = $this->siteBuildRepository->findJob($jobId);
        $status = (string) ($job['status'] ?? '');
        if (in_array($status, ['running', 'completed'], true)) {
            @flock($dispatchLockHandle, LOCK_UN);
            @fclose($dispatchLockHandle);
            return;
        }
        ...
        $workerAlive = false;
        if ($workerPid > 0) {
            if (@\function_exists('posix_kill') && @\posix_kill($workerPid, 0)) {
                $workerAlive = true;
            } elseif (!@\function_exists('posix_kill') && @\is_dir('/proc/' . $workerPid)) {
                $workerAlive = true;
            }
        }

        if ($dispatchCode === 0 && ($workerAlive || in_array($status, ['failed'], true))) {
            @flock($dispatchLockHandle, LOCK_UN);
            @fclose($dispatchLockHandle);
            return;
        }
        ...
        $this->siteBuildRepository->updateJob($jobId, [
            'status' => 'failed',
            'current_step' => 'failed',
            'error_message' => 'failed to start site build worker process (code=' . (string) $dispatchCode . ', pid=' . (string) $workerPid . ', out=' . json_encode($dispatchOutput, JSON_UNESCAPED_UNICODE) . ', status=' . $status . ')',
            'finished_at' => date('Y-m-d H:i:s'),
        ]);
