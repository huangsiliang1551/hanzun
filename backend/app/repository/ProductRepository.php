...
final class ProductRepository
{
    ...
    private function listFromDatabase(PDO $pdo, array $query): array
    {
        ...
        $statement = $pdo->prepare(
            'SELECT p.id, p.category_id, p.sku, p.name_zh, p.summary_zh, p.business_status, p.publish_status, p.translation_status, p.seo_status, p.is_home_featured, p.manual_sort, p.slug, p.publish_time, 0 AS views_count, c.name_zh AS category_name
             FROM products p
        ...
        if ($pdo instanceof PDO && !$this->preferRuntimeStorage()) {
            ...
            $statement = $pdo->query(
                'SELECT id, parent_id, name_zh, slug, sort, is_enabled
                 FROM product_categories' . $conditions . '
