...
final class AdminUserRepository
{
    ...
    public function updateLoginMetadata(int $id, string $loginAt, string $loginIp): ?array
    {
        if ($id <= 0) {
            return null;
        }

        $loginAt = trim($loginAt);
        ...
        if (RuntimeStorage::enabled()) {
            ...
            foreach ($items as $index => $item) {
                ...
                $item['last_login_at'] = $loginAt !== '' ? $loginAt : null;
                $item['last_login_ip'] = $loginIp !== '' ? $loginIp : null;
                $item['updated_at'] = date('Y-m-d H:i:s');
                $items[$index] = $item;
                $this->userStore()->put($items);
        ...
        if ($pdo instanceof PDO) {
            ...
            $statement->execute([
                'id' => $id,
                'last_login_at' => $loginAt !== '' ? $loginAt : null,
                'last_login_ip' => $loginIp !== '' ? $loginIp : null,
            ]);
