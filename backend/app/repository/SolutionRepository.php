...
{
    private const CATEGORY_TABLE = 'solution_categories';
    ...
    public function findBySlug(string $slug): ?array
    {
        ...
        $pdo = DatabaseManager::instance()->connection();
        if ($pdo instanceof PDO && !$this->preferRuntimeStorage()) {
            $statement = $pdo->prepare('SELECT * FROM solutions WHERE slug = :slug LIMIT 1');
    ...
    private function listFromDatabase(PDO $pdo, array $query): array
    {
        ...
        $statement = $pdo->prepare(
            'SELECT s.id, s.category_id, s.name_zh, s.publish_status, s.translation_status, s.seo_status, s.is_home_featured, s.manual_sort, s.slug, s.publish_time, s.views_count, c.name_zh AS category_name
             FROM solutions s
