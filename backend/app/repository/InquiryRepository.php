...
final class InquiryRepository
{
    public function listInquiries(): array
    {
        ...
        if ($pdo instanceof PDO) {
            ...
            $statement = $pdo->query(
                'SELECT i.id, i.source, i.session_id, i.primary_contact_type, i.primary_contact_value, i.customer_name, i.company_name, i.country_code, i.language_code, i.product_interest, i.solution_interest, i.requirement_summary, i.inquiry_score, i.status, ' . $inquiryArchiveSelect . ', i.assigned_to, i.assigned_to_name, i.first_response_at, i.created_at, i.updated_at,
    ...
    public function findInquiry(int $id): ?array
    {
        ...
        if ($pdo instanceof PDO) {
            ...
            $statement = $pdo->prepare(
                'SELECT i.id, i.source, i.session_id, i.primary_contact_type, i.primary_contact_value, i.customer_name, i.company_name, i.country_code, i.language_code, i.product_interest, i.solution_interest, i.requirement_summary, i.inquiry_score, i.status, ' . $inquiryArchiveSelect . ', i.assigned_to, i.assigned_to_name, i.first_response_at, i.browse_traces, i.change_logs, i.follow_ups, i.created_at, i.updated_at,
                        cs.source_page, cs.utm_source, cs.last_message_at
                 FROM inquiries i
                 LEFT JOIN chat_sessions cs ON cs.id = i.session_id
                 WHERE i.id = :id
    ...
    public function updateInquiry(int $id, array $payload): ?array
    {
        ...
        if ($this->preferRuntimeStorage()) {
            ...
            foreach ($inquiries as $index => $inquiry) {
                ...
                $inquiries[$index]['assigned_to_name'] = $next['assigned_to_name'] ?? null;
        ...
        if ($pdo instanceof PDO) {
            ...
            $statement->bindValue(':assigned_to_name', $next['assigned_to_name'] ?? null);
    ...
    public function createInquiry(array $payload): array
    {
        ...
        if ($pdo instanceof PDO) {
            $statement = $pdo->prepare(
                'INSERT INTO inquiries (source, session_id, primary_contact_type, primary_contact_value, customer_name, company_name, country_code, language_code, product_interest, solution_interest, requirement_summary, inquiry_score, status, assigned_to, assigned_to_name, browse_traces, change_logs, follow_ups, source_page, utm_source, last_message_at, created_at, updated_at)
                 VALUES (:source, :session_id, :primary_contact_type, :primary_contact_value, :customer_name, :company_name, :country_code, :language_code, :product_interest, :solution_interest, :requirement_summary, :inquiry_score, :status, :assigned_to, :assigned_to_name, :browse_traces, :change_logs, :follow_ups, :source_page, :utm_source, :last_message_at, NOW(), NOW())'
            );
            $statement->execute([
