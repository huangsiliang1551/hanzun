...
final class PublicChatRepository
{
    ...
    /**
     * 按 client_id 的 hash 前缀找回最近的活跃会话。
     * session_code 格式: web-{clientHash}-{random}
     * 用于同一用户多次访问/多页访问时找回已有会话，避免每次生成新会话。
     */
    public function findLatestSessionCodeByClientHash(string $clientHash): ?string
    {
        ...
        $prefix = 'web-' . $clientHash . '-';

        if ($this->preferRuntimeStorage()) {
            $latest = null;
            $latestAt = '';
            foreach ($this->loadRuntimeConversations() as $record) {
                $code = (string) ($record['session_code'] ?? '');
        ...
        $pdo = $this->databaseConnection();
        if ($pdo instanceof PDO) {
            $archiveWhere = $this->hasTableColumn($pdo, 'chat_sessions', 'archive_status')
            ...
            $row = $statement->fetch();
            return is_array($row) ? (string) ($row['session_code'] ?? '') : null;
        }
    ...
    public function appendVisitorEvent(string $sessionCode, array $event): array
    {
        ...
        $normalizedEvent = [
            ...
            'country_code' => strtoupper((string) ($event['country_code'] ?? '')),
        ];
        ...
        $pdo = $this->databaseConnection();
        if ($pdo instanceof PDO) {
            $columns = ['session_code', 'page', 'title', 'referrer', 'visited_at', 'language_code'];
            $placeholders = [':session_code', ':page', ':title', ':referrer', ':visited_at', ':language_code'];
            $params = [
                'session_code' => $sessionCode,
            ...
            if ($this->hasTableColumn($pdo, 'visitor_events', 'country_code')) {
                $columns[] = 'country_code';
                $placeholders[] = ':country_code';
                $params['country_code'] = $normalizedEvent['country_code'];
            }
            $statement = $pdo->prepare(
                'INSERT INTO visitor_events (' . implode(', ', $columns) . ')
    ...
    public function listVisitorEvents(string $sessionCode): array
    {
        ...
        $pdo = $this->databaseConnection();
        if ($pdo instanceof PDO) {
            $selectFields = 'page, title, referrer, visited_at, language_code';
            if ($this->hasTableColumn($pdo, 'visitor_events', 'country_code')) {
