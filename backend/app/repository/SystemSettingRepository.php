...
{
    private const MACHINE_PROTOCOL_FRAGMENTS = [
        ...
        'Return JSON only with keys: reply, intent_code, contains_contact_info, contact_name, company_name, email, phone, whatsapp, country_code, product_interest, solution_interest, requirement_summary.',
        'You must still return the required JSON object with reply, intent_code, contains_contact_info, contact fields, product_interest, solution_interest, and requirement_summary.',
        'Return JSON only.',
    ];
    ...
    {
        $normalized = $this->stripMachineProtocol(trim($value));
        if ($normalized === '') {
            return $this->stripMachineProtocol($default);
        }
    ...
    private function stripMachineProtocol(string $value): string
    {
        $normalized = $value;
        foreach (self::MACHINE_PROTOCOL_FRAGMENTS as $fragment) {
            $normalized = str_replace($fragment, '', $normalized);
        }
