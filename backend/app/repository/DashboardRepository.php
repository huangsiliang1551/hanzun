...
final class DashboardRepository
{
    ...
    public function contentPvSummary(): array
    {
        $pdo = DatabaseManager::instance()->connection();
        if (!($pdo instanceof \PDO)) {
            return [
        ...
        $statement = $pdo->prepare(
            "SELECT
                (SELECT COALESCE(SUM(COALESCE(views_count, 0)), 0) FROM products) AS product_pv,
                (SELECT COALESCE(SUM(COALESCE(views_count, 0)), 0) FROM solutions) AS solution_pv,
                (SELECT COALESCE(SUM(COALESCE(views_count, 0)), 0) FROM news) AS news_pv,
                (SELECT COALESCE(SUM(COALESCE(views_count, 0)), 0) FROM cases) AS case_pv"
        );
        $statement->execute();
    ...
    public function contentTopEntitiesByType(string $type, int $limit = 5): array
    {
        ...
        $configs = [
            'product' => [
                ...
                'slug_field' => 'slug',
                'title_field' => 'name_zh',
                'route_prefix' => '/zh/products/',
            ],
            'solution' => [
                'table' => 'solutions',
                'slug_field' => 'slug',
                'title_field' => 'name_zh',
                'route_prefix' => '/zh/solutions/',
            ],
            'news' => [
                'table' => 'news',
                'slug_field' => 'slug',
                'title_field' => 'title_zh',
                'route_prefix' => '/zh/news/',
            ],
            'case' => [
                'table' => 'cases',
                'slug_field' => 'slug',
                'title_field' => 'title_zh',
                'route_prefix' => '/zh/cases/',
            ],
        ];
        ...
        $statement = $pdo->prepare(
            "SELECT
                CONCAT(:route_prefix, {$config['slug_field']}, '.html') AS landing_page,
                COALESCE({$config['title_field']}, '') AS title,
                COALESCE(views_count, 0) AS pv
             FROM {$config['table']}
             ORDER BY pv DESC, id DESC
             LIMIT {$safeLimit}"
        );
        $statement->bindValue(':route_prefix', $config['route_prefix']);
        $statement->execute();
