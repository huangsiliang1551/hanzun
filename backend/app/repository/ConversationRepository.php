...
final class ConversationRepository
{
        ...
        if ($pdo instanceof PDO) {
            ...
            if (is_array($conversation)) {
                $messageStatement = $pdo->prepare(
                    'SELECT id, message_role AS role, content, created_at, message_language, translated_text, intent_code, contains_contact_info, extracted_entities_json
                     FROM chat_messages
    ...
    public function updateMessageTranslation(int $messageId, string $translatedText): bool
    {
        if ($messageId <= 0) return false;
        $pdo = DatabaseManager::instance()->connection();
        if (!$pdo instanceof PDO) return false;
        $st = $pdo->prepare('UPDATE chat_messages SET translated_text = :txt WHERE id = :id');
        return $st->execute(['txt' => $translatedText, 'id' => $messageId]);
    }
