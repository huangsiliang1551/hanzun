...
{
    private const TABLE = 'cases';
    private const CATEGORY_TABLE = 'case_categories';
    private const CATEGORY_TRANSLATION_TABLE = 'case_category_translations';
    ...
    public function create(array $payload): array
    {
        ...
        if ($pdo instanceof PDO) {
            $statement = $pdo->prepare(
                'INSERT INTO ' . self::TABLE . ' (category_id, title_zh, summary_zh, content_zh, country_code, case_tags, related_solution_ids, related_product_ids, publish_status, translation_status, seo_status, is_home_featured, manual_sort, slug, seo_title, seo_keywords, seo_description, publish_time, created_by, updated_by, created_at, updated_at)
                 VALUES (:category_id, :title_zh, :summary_zh, :content_zh, :country_code, :case_tags, :related_solution_ids, :related_product_ids, :publish_status, :translation_status, :seo_status, :is_home_featured, :manual_sort, :slug, :seo_title, :seo_keywords, :seo_description, :publish_time, :created_by, :updated_by, NOW(), NOW())'
            );
            $statement->execute([
    ...
    private function listFromDatabase(PDO $pdo, array $query): array
    {
        ...
        if ($query['publish_status'] !== '') {
            $conditions[] = 'c.publish_status = :publish_status';
            $params['publish_status'] = $query['publish_status'];
        }
        if ($query['country_code'] !== '') {
            $conditions[] = 'c.country_code = :country_code';
        ...
        if ($query['is_home_featured'] !== null) {
            $conditions[] = 'c.is_home_featured = :is_home_featured';
            $params['is_home_featured'] = (int) $query['is_home_featured'];
        }
        if ($query['keyword'] !== '') {
            $conditions[] = '(c.title_zh LIKE :keyword OR c.case_tags LIKE :keyword)';
            $params['keyword'] = '%' . $query['keyword'] . '%';
        }

        $whereSql = $conditions === [] ? '' : ' WHERE ' . implode(' AND ', $conditions);
        $countStatement = $pdo->prepare('SELECT COUNT(*) FROM ' . self::TABLE . ' c' . $whereSql);
        $countStatement->execute($params);
        $total = (int) $countStatement->fetchColumn();

        $sortMap = [
            'manual_sort' => 'c.manual_sort',
        ...
        $statement = $pdo->prepare(
            'SELECT c.id, c.category_id, c.slug, c.title_zh, c.country_code, c.case_tags, c.publish_status, c.translation_status, c.seo_status, c.is_home_featured, c.manual_sort, c.publish_time, c.views_count, cat.name_zh AS category_name
             FROM ' . self::TABLE . ' c
             LEFT JOIN ' . self::CATEGORY_TABLE . ' cat ON cat.id = c.category_id'
            . $whereSql .
            ' ORDER BY ' . $sortMap[$query['sort_field']] . ' ' . strtoupper((string) $query['sort_order']) . ', c.id DESC
