...
class SettingController extends BaseAdminController
{
    ...
    public function updateSite(Request $request): array
    {
        return $this->success($this->settingService->updateSiteConfig([
            ...
            'hero_cta_primary' => $request->input('hero_cta_primary'),
            'hero_cta_secondary' => $request->input('hero_cta_secondary'),
            'notice_image_url' => $request->input('notice_image_url'),
            'notice_title' => $request->input('notice_title'),
            'notice_content' => $request->input('notice_content'),
            'service_support_title' => $request->input('service_support_title'),
            'service_support_line_1' => $request->input('service_support_line_1'),
            'service_support_line_2' => $request->input('service_support_line_2'),
            'service_support_line_3' => $request->input('service_support_line_3'),
            'service_support_line_4' => $request->input('service_support_line_4'),
        ]), [], '站点设置已更新');
    }
    ...
    {
        return $this->success($this->settingService->updateDeepseekConfig($this->buildDeepseekPayload($request)), [], 'AI 设置已更新');
    }

    public function batchTranslatePhrases(): array
    {
        $result = $this->settingService->batchTranslatePhrases();
        return $this->success($result, [], '站点短语批量翻译已触发');
    }
