...
if (str_starts_with($path, '/api/') || $path === '/health') {
    // Keep /api prefix — backend publicapi routes already include /api prefix
...
if ($path === '/') {
    if (outputFile($projectRoot . $defaultHome)) {
        return true;
    }

    if (outputFile($projectRoot . '/zh/index.html')) {
        return true;
    }

    if (outputFile($projectRoot . '/en/index.html')) {
        return true;
    }
...
// NOTE: Keep compatibility for direct file access to a root index when provided by callers.
if (outputFile($projectRoot . $path)) {
    return true;
}
