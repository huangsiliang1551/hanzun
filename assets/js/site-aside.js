...
(function () {
    ...
    function formatSupportMessageTime(value) {
        ...
        var locale = R.getContentLanguage(R.normalizedLang((body && body.dataset.lang) || "zh")) === "en" ? "en-GB" : "zh-CN";
        ...
        try {
            ...
            var yearPrefix = sameYear ? "" : parsed.getFullYear() + "-";
        ...
        if (type === "assistant") {
            ...
            if (sources.length) {
                ...
                sources.forEach(function (item) {
                    ...
                    sourceTitle.textContent = item.title || R.getLocalizedSupportText("参考资料", "Reference");
                    sourceItem.appendChild(sourceTitle);
                    var sourceTypeLabel = localizeSupportSourceType(item.sourceType);
                    if (sourceTypeLabel) {
    ...
    if (supportPromptButtons.length) {
        supportPromptButtons.forEach(function (button) {
            button.addEventListener("click", function () {
                var prompt = button.dataset.supportPrompt || button.textContent.trim();
    ...
    if (supportForm && supportInput) {
        ...
        supportInput.addEventListener("keydown", async function (event) {
