...
(function () {
    ...
    /* ───────────────────────── 内容字段解析（共享 site-nav.js 的实现） ─────────────────────────
       site-nav.js 加载在前，已将 resolveContentTitle / resolveContentSummary / resolveContentImage /
       resolveFallbackImage / buildStaticPublicHref / mapStaticPublicHref / resolvePublicHref 暴露到
       window.HanzunRuntime，本模块直接通过 R.* 引用，避免重复实现。 */
    var resolveContentTitle = R.resolveContentTitle;
    var resolveContentSummary = R.resolveContentSummary;
    var resolveContentImage = R.resolveContentImage;
    var resolveFallbackImage = R.resolveFallbackImage;
    var buildStaticPublicHref = R.buildStaticPublicHref;
    var mapStaticPublicHref = R.mapStaticPublicHref;
    var resolvePublicHref = R.resolvePublicHref;
    ...
    function hydrateFooterFeaturedColumn(host, heading, items, fallbackHref) {
        ...
        var featured = items.filter(function (item) {
            return item && item.is_home_featured === 1;
        });
        var visible = featured.length ? featured : items.filter(Boolean);
        host.innerHTML = "<h3>" + R.escapeHtml(heading) + "</h3>" +
            visible.slice(0, 5).map(function (item) {
    ...
    async function hydratePublicSite(requestedCode) {
        if (isStaticGeneratedPublicPage) {
            ...
            try {
                var staticResult = await R.getPublicApi("/api/site/bootstrap?lang=" + encodeURIComponent(R.toApiLanguageCode(staticRequestedCode)));
                var staticPayload = (staticResult && staticResult.data) || {};
                var staticHomepage = staticPayload.homepage || {};
                var staticFeaturedProducts = (homepageSectionByKey(staticHomepage, "featured_products") || {}).items || [];
                var staticFeaturedSolutions = (homepageSectionByKey(staticHomepage, "featured_solutions") || {}).items || [];
                var staticFeaturedCases = (homepageSectionByKey(staticHomepage, "customer_cases") || {}).items
                    || (homepageSectionByKey(staticHomepage, "featured_cases") || {}).items
                    || (homepageSectionByKey(staticHomepage, "case_list") || {}).items
                    || [];
                var staticFeaturedNews = (homepageSectionByKey(staticHomepage, "company_news") || {}).items
                    || (homepageSectionByKey(staticHomepage, "news_list") || {}).items
                    || [];
                var staticResolvedCode = (staticResult && staticResult.meta && staticResult.meta.language && staticResult.meta.language.resolved_code) || R.toApiLanguageCode(staticRequestedCode);

                if (staticPayload && typeof staticPayload === "object") applyRuntimePayload(staticPayload);
                hydrateSiteConfig(staticPayload.site);
                if (typeof R.loadNavigation === "function") R.loadNavigation(staticResolvedCode);
                hydrateHeroSection(staticHomepage);
                hydrateSolutionsGrid(staticFeaturedSolutions);
                hydrateProductsShowcase(staticFeaturedProducts);
                hydrateCasesBoard(staticFeaturedCases);
                hydrateNewsGrid(staticFeaturedNews);
                hydrateContactPanels(staticPayload.contact);
                hydrateFooterFeaturedColumn(R.footerFeaturedProducts, R.getLocalizedRuntimeCopy("热门产品", "Popular Products"), staticFeaturedProducts, "/products");
                hydrateFooterFeaturedColumn(R.footerFeaturedSolutions, R.getLocalizedRuntimeCopy("热门方案", "Popular Solutions"), staticFeaturedSolutions, "/solutions");
                if (typeof R.hydrateCertificatesGrid === "function") R.hydrateCertificatesGrid(staticPayload.certificates || []);
                if (typeof R.hydrateTeamStrip === "function") R.hydrateTeamStrip(staticPayload.team_members || []);
            } catch (e) {
        ...
        try {
            ...
            hydrateFooterFeaturedColumn(R.footerFeaturedSolutions, R.getLocalizedRuntimeCopy("热门方案", "Popular Solutions"), featuredSolutions, "/solutions");
            R.initProgressiveMedia();
            R.populateSalesContactMenus();

            // T1: Hero media (video or image)
            var heroMedia = document.querySelector(".hero-image");
            if (heroMedia && payload.site && payload.site.hero_image_url) {
                var heroUrl = payload.site.hero_image_url;
                var heroAlt = (payload.site && payload.site.hero_image_alt) || "";
                if (heroMedia.tagName && heroMedia.tagName.toLowerCase() === "video") {
                    var heroSource = heroMedia.querySelector("source");
                    if (heroSource) {
                        heroSource.src = heroUrl;
                        var ext = (heroUrl.split(".").pop() || "").toLowerCase();
                        heroSource.type = ext === "mp4" ? "video/mp4" : "video/webm";
                        heroMedia.load();
                    } else {
                        heroMedia.src = heroUrl;
                    }
                    if (heroAlt) heroMedia.setAttribute("aria-label", heroAlt);
                } else {
                    heroMedia.src = heroUrl;
                    heroMedia.alt = heroAlt || heroMedia.alt;
                }
            }
    ...
    // 当前生成链路下本文件仅注入首页，直接执行首页水合。
    // 这样即使旧 runtime 没有正确暴露 isStaticGeneratedPublicPage，也不会把首页卡在 data-hydrating 隐藏态。
    hydratePublicSite(initialLanguage);
    void trackStaticDetailPageView();

    /* ───────────────────────── Hero 打字机 + 视频懒加载 ─────────────────────────
       仅首页 hero 区域生效：
       1. 视频懒加载：source data-src → 进入视口附近才赋值 src 并播放，避免首屏带宽占用
       2. 标题打字机：data-hero-typewriter 元素文本逐字写出，附带闪烁光标
       3. 副标题/按钮：data-hero-followup 元素初始隐藏，打字完成后淡入显示
       无障碍：尊重 prefers-reduced-motion，弱化动效直接显示完整文案 */
    function initHeroTypewriterAndLazyVideo() {
        var heroVideo = document.querySelector('.hero-video[data-hero-lazy-video]');
        var typewriterEl = document.querySelector('.hero-title[data-hero-typewriter]');
        var followups = Array.prototype.slice.call(document.querySelectorAll('[data-hero-followup]'));
        ...
        if (heroVideo) {
            ...
            if (heroVideo.readyState >= 2) {
                ...
                heroVideo.addEventListener('loadeddata', ensurePlay, { once: true });
                heroVideo.load();
            }
        }

        // 2/3. 打字机 + 副标题/按钮淡入
        var revealFollowups = function () {
            followups.forEach(function (el) { el.classList.add('is-visible'); });
        };
    ...
    // 首页才执行（hero 区域仅在首页存在）
    if (document.querySelector('.hero-title[data-hero-typewriter]') || document.querySelector('.hero-video[data-hero-lazy-video]')) {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initHeroTypewriterAndLazyVideo);
        } else {
            initHeroTypewriterAndLazyVideo();
        }
    }
})();
