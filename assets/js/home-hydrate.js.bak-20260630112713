...
(function () {
    ...
    function hydrateFooterFeaturedColumn(host, heading, items, fallbackHref) {
        ...
        var visible = items.filter(function (item) {
            return item && item.is_home_featured === 1 && item.has_cover_image;
        });
    ...
    function initHeroTypewriterAndLazyVideo() {
        ...
        // 1. 视频懒加载
        if (heroVideo) {
            var sources = Array.prototype.slice.call(heroVideo.querySelectorAll('source[data-src]'));
            var videoLoaded = false;
            var loadVideo = function () {
                if (videoLoaded || sources.length === 0) return;
                videoLoaded = true;
                // 把所有 source 的 data-src 赋值给 src，浏览器自动选支持的格式
            ...
            if ('IntersectionObserver' in window) {
                var obs = new IntersectionObserver(function (entries) {
                    entries.forEach(function (e) {
                        if (e.isIntersecting) { loadVideo(); obs.unobserve(heroVideo); }
                    });
                }, { rootMargin: '200px' });
        ...
        // 缓存原始文本到 data-text 以便 SSR/无JS时仍可读
        ...
        var charIndex = 0;
        var charDelay = 75; // 每字 75ms
        var startDelay = 350; // 首屏稍等 350ms 再开始打字

        var typeNext = function () {
            if (charIndex >= fullText.length) {
                typewriterEl.classList.add('is-done');
                // 250ms 后移除光标 + 淡入副标题和按钮
                setTimeout(function () {
